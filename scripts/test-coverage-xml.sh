...
if [ -n "$1" ]; then
    coverage run -m pytest $TEST_SRC --branch --color=yes -vv --environment $TEST_ENVIRONMENT --no-provision --cleanup-docker
else
    coverage run -m pytest tests/unit --branch --color=yes -vv --environment $TEST_ENVIRONMENT --no-provision
    coverage run -m pytest tests/integration --branch --color=yes -vv --environment $TEST_ENVIRONMENT --no-provision --cleanup-docker
fi
