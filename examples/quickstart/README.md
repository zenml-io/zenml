...
At the core of ZenML, you will find the concepts of **Pipelines**. These 
pipelines provide our users a simple python interface to design their ML 
workflows. For instance, the training pipeline that we will use in this example 
looks like this:
...
Each pipeline consists of several steps which we will cover in the next section.

#### Steps

Similar to pipelines, ZenML also provides a simple python interface for 
stand-alone **Steps**. These steps are designed consume input artifacts and 
generate output artifacts. As an example, we can take a closer look at one 
