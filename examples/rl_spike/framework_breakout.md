# RL spike — framework survey brief (planning, 2026-07-08, rev. 15)
...
*Rev. 15 (2026-07-09, late evening): **C1 is done** (branch `spike/c1-verifiers-eval`, merged back into `misc/rl-spike` — deliverables: `verifiers_c2/c1_eval_pipeline.py` + [`C1_EVAL.md`](verifiers_c2/C1_EVAL.md); FINDINGS.md Theme 5 extended; BREAKAGE_LOG entries 25–27). Verdict: the wrapping earns its keep for *campaigns*, not single evals — cross-model runs (gpt-5-mini `d30d6da8` mean 1.00 incl. the OOD tasks, gpt-5-nano `c04577b1` mean 0.94 with `word_ladder`'s scorer error visible end-to-end) live on staging as structurally identical runs with per-shard rollout artifacts, token/timing metadata, and a dashboard-rendered report; mapped shards overlapped even on the local orchestrator. Sub-answers: (a) `asyncio.run` in a step behaves, but verifiers installs signal handlers at environment construction and dies in ZenML's worker threads (entry 26, three-line workaround); (b) the HF-datasets materializer story is blocked outright — zenml's huggingface integration pins `datasets<4.0.0` vs the ecosystem's 4.x, cloudpickle fallback with a py-version warning (entry 25), plus Arrow's one-schema rule forces JSON-string columns; (c) answered in the verdict. Bonus: the entry-16 ambiguity recurred in our own C2-hardened code (one line above the `try:`) — ten plausible completions scored 0.0 with nothing flagged, now fixed with C2 parity re-verified 18/18. C3 remains the open Track C entry.*

*Rev. 14 (2026-07-09, after the G1 build — see `GEPA_G1.md`): **G1 is done** (gepa==0.1.1 on the unchanged harness, hosted API models, zero GPU: one-line seed → 8-candidate tree, mean reward 0.515 → 0.839 local and 0.385 → 0.946 replicated on staging with dashboard-visible artifacts incl. an HTMLString evolution report; verifier byte-identical to the GRPO baseline). Question (a) answered YES with a visibility trade — a framework-owned inner loop coarsens ZenML's unit of visibility from episode to optimization-run; question (b) answered NO natively — the Pareto candidate tree is carried inside artifacts, escalated as a core artifact-model question; question (c) works with the reflection LM as a callable (litellm not required; the callable is the natural cost-attribution point). Sharpened honesty note: with the full cheatsheet, gpt-5-nano already scores 1.0/1.0 — on this task set prompts saturate before weights are needed. New BREAKAGE_LOG entries 22–24.*

*Rev. 13 (2026-07-09, evening): **B3 is done** (branch `spike/b3-exporter` off B1's branch, pushed, merges nowhere — deliverables: [`B3_EXPORTER.md`](https://github.com/zenml-io/zenml/blob/spike/b3-exporter/examples/harbor_agent_evals/B3_EXPORTER.md) + `export_episodes.py`; FINDINGS.md Theme 5 extended). Verdict: the eval→training lineage edge IS expressible and was demonstrated on B1's real staging campaigns — the accumulating `rl_episode_candidates` dataset grew through six versions (idempotent re-export, cache-hit reruns add nothing, a genuine re-attempt joins by trial identity across runs), with export inputs verified identical to campaign output artifact versions. But three walls: `List[ArtifactVersionResponse]` step inputs fall through to raw parameters and `.map()` refuses historical artifacts (workaround: a plain loop, one step per shard — collides with E3's per-step cost at 89-task scale); #5029's `trial_identity` fragments across task-specification routes (dataset resolver stamps `source:` into the ref; local paths hash absolute paths); and no layer records the resolved sandbox image (the stack-config fallback is provably wrong for `docker_image`-pinned tasks). Keep verdict — nothing scraped Harbor internals. Three escalations collected in `B3_EXPORTER.md`. G1 finished in parallel the same evening — its rev note lands separately.*
...
**Status: DONE 2026-07-09** — see rev. 13 note; exporter + findings on branch `spike/b3-exporter` (`export_episodes.py`, `B3_EXPORTER.md`), FINDINGS.md Theme 5. The stub `grpo_update` consumer (smallest-shape item 2's second half) was deliberately not built, per the entry's own scoping.
...
#### C1 · verifiers environment eval as a mapped ZenML campaign — **DONE (7/9, see verifiers_c2/C1_EVAL.md; FINDINGS.md Theme 5; entries 25–27)**

*Outcome: the keep branch, with a scoped verdict — the wrapping adds nothing for a single eval but earns its keep for campaigns (versioned per-shard rollout artifacts, cross-model run history, dashboard-rendered report, independently retryable shards that overlapped even on the local orchestrator). Ran against hosted API models (gpt-5-mini/gpt-5-nano, dry variant — A1's warm endpoint not needed); nano produced the reward spread (0.94 mean, `word_ladder` 0.40 with its scorer error visible end-to-end). Mechanical answers: `asyncio.run` in a step behaves; verifiers' signal-handler installation at env construction does not (entry 26); the HF-datasets materializer path is version-blocked, `datasets<4.0.0` vs the ecosystem's 4.x (entry 25).*
...
#### G1 · GEPA on the existing spike harness — same loop, different update rule — **DONE (7/9, see `GEPA_G1.md`)**
...
8. ~~**C1**, then~~ **C1 done (7/9, hosted-API dry variant — no warm endpoint needed)**; **C3** remains, gated only on wanting A1's warm endpoint up.
