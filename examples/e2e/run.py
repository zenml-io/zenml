...
def main(
    ...
    if not only_inference and not only_deployment:
        pipeline_args["config_path"] = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "configs",
            "train_config.yaml",
        ...
        pipeline_args["run_name"] = (
            f"e2e_use_case_training_vertex_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        )
        e2e_use_case_training_vertex.with_options(**pipeline_args)(
            **run_args_train
        )
