...
Launches the Harbor evaluation campaign pipeline with a given config.
"""
...
def _collect_runtime_env_vars() -> Dict[str, str]:
    ...
    for key in ("OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DAYTONA_API_KEY"):
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    parser.add_argument(
        "--forward-env",
        action="store_true",
        help="Forward local API keys (OPENAI, ANTHROPIC, DAYTONA) to remote pods",
    )
