...
def evaluate_prompts_with_test_cases(
    ...
    logger.info(f"🧪 Running comprehensive evaluation: {len(prompt_variants)} prompts × {len(test_cases)} test cases")
    ...
    for prompt_idx, system_prompt in enumerate(prompt_variants):
        ...
        for test_case in test_cases:
            case_result = _run_single_test_case(
                dataset_df, system_prompt, test_case, agent_config, llm_judge, prompt_id
            )
            prompt_results.append(case_result)
        
        # Aggregate results for this prompt
        prompt_summary = _analyze_prompt_performance(prompt_results, prompt_id, system_prompt)
...
def compare_agent_prompts(
    ...
    logger.info(f"🧪 Testing {len(prompt_variants)} prompt variants on dataset with {len(dataset_df)} rows")
    
    results = []
    
    for i, system_prompt in enumerate(prompt_variants):
        prompt_id = f"variant_{i+1}"
        logger.info(f"🔄 Testing prompt variant {i+1}/{len(prompt_variants)}")
        ...
        try:
            ...
            test_agent = Agent(
                ...
                model_settings=ModelSettings(parallel_tool_calls=False),  # Disable parallel to avoid hanging
            )
...
Focus on data quality, key patterns, and actionable insights."""
            ...
            result_metrics = {
                ...
                "tables_generated": len(deps.output) - 1,  # Exclude main dataset
                "error": None,
            }
            
            logger.info(f"✅ Variant {i+1}: Score={eda_report.data_quality_score:.1f}, Tools={len(deps.query_history)}, Time={execution_time:.1f}s")
    ...
    if successful_results:
        ...
        for result in successful_results:
            ...
            speed_score = max(0, 100 - result["execution_time_seconds"] * 10)  # Penalty for slowness
            thoroughness_score = (result["key_findings_count"] * 10 + 
        ...
        logger.info(f"🏆 Best performing prompt: {best_prompt_id} (score: {best_result['composite_score']:.1f})")
    else:
        best_prompt_id = "none"
...
def _generate_prompt_recommendations(results: List[Dict[str, Any]]) -> List[str]:
    ...
    if not successful_results:
        return ["All prompts failed - check agent configuration and error messages"]
    
    # Performance analysis
    avg_time = sum(r["execution_time_seconds"] for r in successful_results) / len(successful_results)
    avg_quality = sum(r["data_quality_score"] for r in successful_results) / len(successful_results)
    
    if avg_time > 30:
        recommendations.append("Consider shorter, more focused prompts to reduce execution time")
    
    if avg_quality < 70:
        recommendations.append("Consider more specific instructions for data quality assessment")
    ...
    if quality_variance > 20:
        recommendations.append("High variance in quality scores - consider more consistent prompt structure")
    else:
        recommendations.append("Quality scores are consistent across prompts - good stability")
    
    # Tool usage analysis
    tool_calls = [r["tool_calls_made"] for r in successful_results]
    if max(tool_calls) - min(tool_calls) > 3:
        recommendations.append("Variable tool usage detected - consider standardizing analysis steps")
    
    # Success rate analysis
    success_rate = len(successful_results) / len(results) * 100
    if success_rate < 80:
        recommendations.append("Low success rate - review prompt complexity and error handling")
    else:
        recommendations.append(f"Good success rate ({success_rate:.0f}%) - prompts are robust")
...
def _create_llm_judge(base_model: str) -> Optional[Agent]:
    ...
    try:
        ...
        judge_model = "gpt-4o" if "gpt" in base_model else "claude-3-5-sonnet-20241022"
...
Be objective and consistent in your evaluations."""
        ...
        return Agent(
            f"openai:{judge_model}" if "gpt" in base_model else f"anthropic:{judge_model}",
...
def _run_single_test_case(
    ...
    try:
        ...
        if llm_judge:
            judge_evaluation = _get_llm_judge_scores(llm_judge, test_case, case_result)
...
def _get_llm_judge_scores(llm_judge: Agent, test_case: Dict, case_result: Dict) -> Dict:
...
Please evaluate this EDA response and provide scores in the requested JSON format."""
        ...
        try:
            ...
            return {"judge_overall": 3, "judge_reasoning": "Failed to parse judge response"}
            
    except Exception as e:
        logger.warning(f"LLM judge evaluation failed: {e}")
        return {"judge_overall": 3, "judge_reasoning": f"Evaluation error: {e}"}
...
def _analyze_prompt_performance(results: List[Dict], prompt_id: str, system_prompt: str) -> Dict:
    ...
    avg_scores = {
        ...
        "tool_calls": sum(r["tool_calls_made"] for r in successful_results) / len(successful_results),
        "data_quality_score": sum(r["data_quality_score"] for r in successful_results) / len(successful_results),
        "findings_count": sum(r["findings_count"] for r in successful_results) / len(successful_results),
    }
    ...
    if judge_scores:
        avg_scores.update({
            "judge_accuracy": sum(r["judge_accuracy"] for r in judge_scores) / len(judge_scores),
            "judge_relevance": sum(r["judge_relevance"] for r in judge_scores) / len(judge_scores),
            "judge_completeness": sum(r["judge_completeness"] for r in judge_scores) / len(judge_scores),
            "judge_overall": sum(r["judge_overall"] for r in judge_scores) / len(judge_scores),
        })
    ...
    for result in successful_results:
        ...
        if cat not in categories:
            categories[cat] = {"count": 0, "avg_score": 0, "meets_expectations": 0}
    ...
    for cat in categories:
        ...
        categories[cat]["success_rate"] = categories[cat]["meets_expectations"] / categories[cat]["count"]
    
    # Overall rating
    success_rate = len(successful_results) / len(results)
    avg_judge_score = avg_scores.get("judge_overall", avg_scores.get("data_quality_score", 50) / 20)
...
def _generate_comprehensive_analysis(all_results: List[Dict], test_cases: List[Dict]) -> Dict:
    """Generate final comprehensive analysis of all prompt variants."""
    # Rank prompts by overall performance
    ranked_prompts = sorted(all_results, key=lambda x: (
        x["success_rate"] * 0.4 + 
    ...
    return {
        "evaluation_summary": {
            ...
            "best_prompt_rating": best_prompt["overall_rating"] if best_prompt else "none",
            "categories_tested": list(category_insights.keys()),
        },
        "prompt_rankings": ranked_prompts,
        "category_analysis": category_insights,
        "detailed_results": all_results,
        "recommendations": _generate_advanced_recommendations(all_results, category_insights),
    }


def _generate_advanced_recommendations(all_results: List[Dict], category_insights: Dict) -> List[str]:
    ...
    if avg_success_rate < 0.6:
        recommendations.append("Low overall success rate - consider simplifying prompts or checking tool integration")
    ...
    if time_variance > 30:
        recommendations.append("High variance in execution time - optimize slower prompts for efficiency")
    
    # Quality assessment
    if any("judge_overall" in r["avg_scores"] for r in all_results):
        judge_scores = [r["avg_scores"]["judge_overall"] for r in all_results if "judge_overall" in r["avg_scores"]]
        avg_judge_score = sum(judge_scores) / len(judge_scores)
        
        if avg_judge_score < 3:
            recommendations.append("LLM judge scores are low - review prompt clarity and specificity")
        elif avg_judge_score > 4:
            recommendations.append("Excellent LLM judge scores - prompts are producing high-quality responses")
    ...
    for category, data in category_insights.items():
        if data["prompt_performance"]:
            ...
            if min(cat_scores) < 60:
                recommendations.append(f"'{category}' category shows low scores - consider specialized prompts for this use case")
    
    # Best practices
    best_prompt = max(all_results, key=lambda x: x["success_rate"])
    if best_prompt["success_rate"] > 0.8:
        recommendations.append(f"'{best_prompt['prompt_id']}' shows strong performance - consider it as your baseline")
