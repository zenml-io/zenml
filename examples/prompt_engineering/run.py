...
def run_basic_pipeline(args):
    ...
    template = args.template or "You are a helpful assistant. Answer this question concisely: {question}"
...
def run_comparison_pipeline(args):
    ...
    base_template = args.base_template or "Answer the following question: {question}"
    ...
    for rank in comparison["rankings"]:
        logger.info(f"  Rank {rank['rank']}: {rank['prompt_id'][:8]}... (success rate: {rank['success_rate']:.2%})")
...
def run_few_shot_pipeline(args):
    ...
    base_template = args.base_template or """Answer the question based on the examples provided.
...
Answer:"""
    ...
    logger.info("\nComparison results:")
    for rank in comparison["rankings"]:
        logger.info(f"  {rank['template_preview'][:50]}... - Success rate: {rank['success_rate']:.2%}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run ZenML prompt engineering pipelines")
    
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Basic pipeline
    basic_parser = subparsers.add_parser("basic", help="Run basic prompt development pipeline")
    basic_parser.add_argument("--template", type=str, help="Prompt template to test")
    basic_parser.add_argument("--prompt-type", type=str, default="user", choices=["system", "user", "assistant"])
    basic_parser.add_argument("--task", type=str, help="Task type (e.g., qa, summarization)")
    basic_parser.add_argument("--model", type=str, default="gpt-3.5-turbo", help="Model to use")
    
    # Comparison pipeline
    comparison_parser = subparsers.add_parser("comparison", help="Run prompt comparison pipeline")
    comparison_parser.add_argument("--base-template", type=str, help="Base prompt template")
    comparison_parser.add_argument("--variant-templates", type=str, nargs="+", help="Variant templates to test")
    comparison_parser.add_argument("--model", type=str, default="gpt-3.5-turbo", help="Model to use")
    comparison_parser.add_argument("--dataset", type=str, default="default", choices=["default", "qa_hard", "summarization"])
    
    # Experimentation pipeline
    exp_parser = subparsers.add_parser("experiment", help="Run experimentation pipeline with LLM judge")
    exp_parser.add_argument("--model", type=str, default="gpt-3.5-turbo", help="Model to test prompts with")
    exp_parser.add_argument("--judge-model", type=str, default="gpt-4", help="Model to use as judge")
    exp_parser.add_argument("--prompt-configs", type=str, help="JSON file with prompt configurations")
    
    # Few-shot pipeline
    few_shot_parser = subparsers.add_parser("few-shot", help="Run few-shot prompting pipeline")
    few_shot_parser.add_argument("--base-template", type=str, help="Base template for few-shot prompting")
    few_shot_parser.add_argument("--model", type=str, default="gpt-3.5-turbo", help="Model to use")
    
    # View results
    view_parser = subparsers.add_parser("view", help="View results from previous runs")
    view_parser.add_argument("--pipeline-name", type=str, help="Filter by pipeline name")
    view_parser.add_argument("--num-runs", type=int, default=5, help="Number of runs to show")
    view_parser.add_argument("--show-artifacts", action="store_true", help="Show artifact details")
