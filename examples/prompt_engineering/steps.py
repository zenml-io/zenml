...
import json
import os
from typing import Annotated, Dict, List, Any, Tuple, Optional

from zenml import step
from zenml.logger import get_logger
from zenml.prompts import Prompt, create_prompt_variant, compare_prompts
...
def create_variant_step(
    ...
    **changes: Any
    ...
    logger.info(f"Created variant '{variant_name}' from prompt {base_prompt.prompt_id}")
            ...
            if use_openai:
                if model.startswith("gpt"):
                    ...
                    response = client.chat.completions.create(
                        model=model,
                        messages=[
                            {"role": prompt.prompt_type, "content": formatted_prompt}
                        ],
...
def evaluate_prompt_step(
    ...
    output_lengths = [
        len(r["output"]) for r in test_results 
...
def load_test_dataset_step(
    ...
    logger.info(f"Loaded {len(dataset)} test cases from '{dataset_name}' dataset")
...
def compare_prompts_step(
    ...
    for i, (prompt, prompt_results) in enumerate(zip(prompts, results)):
        ...
        success_rate = successful / len(prompt_results) if prompt_results else 0
        
        avg_length = sum(
            len(r["output"]) for r in prompt_results 
            if r["success"] and r["output"]
    ...
    result = {
        ...
        "summary": {
            ...
            "best_success_rate": comparisons[0]["success_rate"] if comparisons else 0,
            "worst_success_rate": comparisons[-1]["success_rate"] if comparisons else 0
        }
    }
    
    logger.info(f"Comparison complete. Best prompt: {result['best_prompt_id']}")
    ...
    for result in test_results:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=judge_model,
                messages=[
                    {"role": "system", "content": "You are an expert evaluator. Provide honest, detailed evaluations."},
            ...
            logger.error(f"Error evaluating test case {result['test_case_id']}: {e}")
    
    # Calculate aggregate scores
    if evaluations:
        avg_scores = {
            "accuracy": sum(e.get("accuracy", 0) for e in evaluations) / len(evaluations),
            "relevance": sum(e.get("relevance", 0) for e in evaluations) / len(evaluations),
            "clarity": sum(e.get("clarity", 0) for e in evaluations) / len(evaluations),
            "completeness": sum(e.get("completeness", 0) for e in evaluations) / len(evaluations),
            "overall": sum(e.get("overall", 0) for e in evaluations) / len(evaluations)
        }
    else:
        avg_scores = {}
