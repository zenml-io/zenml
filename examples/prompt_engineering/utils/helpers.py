...
def format_comparison_results(results: dict) -> str:
    ...
    for i, question in enumerate(results["test_questions"]):
        ...
        output.append(f"  Q{i+1}: V1.0={v1_score}, V2.0={v2_score} → {test_winner}")
...
def validate_prompt_template(template: str) -> Dict[str, Any]:
    ...
    if not variables:
        ...
        suggestions.append("Consider adding variables like {question} or {context}")
    ...
    instruction_words = ["answer", "explain", "describe", "provide", "generate"]
    has_instructions = any(word in template.lower() for word in instruction_words)
    
    if not has_instructions:
        issues.append("No clear instructions found")
        suggestions.append("Add clear action words like 'explain', 'answer', or 'describe'")
