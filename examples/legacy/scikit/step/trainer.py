...
from zenml.utils import naming_utils, path_utils
...
class MyScikitTrainer(BaseTrainerStep):
    ...
    def __init__(self, c: float = 1.0, kernel: Text = "rbf", **kwargs):
        ...
        super(MyScikitTrainer, self).__init__(
            c=self.c, kernel=self.kernel, **kwargs
        )

    def run_fn(self):
        train_split_patterns = [
            self.input_patterns[split]
            for split in self.split_mapping[trainer_utils.TRAIN_SPLITS]
        ]
        x_train, y_train = self.input_fn(train_split_patterns)

        eval_split_patterns = [
            self.input_patterns[split]
            for split in self.split_mapping[trainer_utils.EVAL_SPLITS]
        ]
        ...
        for split in self.split_mapping[trainer_utils.TEST_SPLITS]:
            ...
            trainer_utils.save_test_results(
                test_results, self.output_patterns[split]
            )
    ...
    def input_fn(self, file_pattern: List[Text]):
        ...
        df = convert_raw_dataset_to_pandas(dataset, self.schema, 100000)
        ...
        y = df[
            [
                x
                for x in df.columns
                if naming_utils.check_if_transformed_label(x)
            ]
        ]
        return X, y
    ...
    def test_fn(self, model, x_eval, y_eval):
        ...
        out_dict.update(
            {k: np.array(v) for k, v in x_eval.to_dict(orient="list").items()}
        )
        return out_dict
