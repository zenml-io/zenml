...
class TorchRegressionTrainer(TorchFeedForwardTrainer):
    def __init__(
        self,
        batch_size: int = 32,
        lr: float = 0.0001,
        ...
        **kwargs,
        ...
        super(TorchRegressionTrainer, self).__init__(
    ...
    def input_fn(self, file_patterns: List[Text]):

        dataset = torch_utils.TFRecordTorchDataset(file_patterns, self.schema)
        loader = torch.utils.data.DataLoader(
            dataset, batch_size=self.batch_size, drop_last=True
        )
        return loader
    ...
    def run_fn(self):
        ...
        for e in range(1, self.epochs + 1):
            ...
            print(
                f"Epoch {e + 0:03}: | MSE: " f"{epoch_loss / step_count:.5f}"
            )
