...
from trainer.gan_functions import (
    CycleGan,
    Discriminator,
    Generator,
    TensorBoardImage,
...
from zenml.utils.naming_utils import (
    check_if_transformed_feature,
...
class CycleGANTrainer(TFFeedForwardTrainer):
    ...
    def model_fn(
        self, train_dataset: tf.data.Dataset, eval_dataset: tf.data.Dataset
    ):
    ...
    def input_fn(self, file_pattern: List[Text]):
        xf_feature_spec = {
            ...
            if check_if_transformed_feature(x) or check_if_transformed_label(x)
        }
