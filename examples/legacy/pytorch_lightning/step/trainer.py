...
from zenml.steps.trainer import TorchFeedForwardTrainer, utils
...
class MyPyTorchLightningTrainer(TorchFeedForwardTrainer):
    ...
    def run_fn(self):
        train_split_patterns = [
            ...
            for split in self.split_mapping[utils.TRAIN_SPLITS]
        ]
        train_dataset = self.input_fn(train_split_patterns)

        eval_split_patterns = [
            self.input_patterns[split]
            for split in self.split_mapping[utils.EVAL_SPLITS]
        ]
