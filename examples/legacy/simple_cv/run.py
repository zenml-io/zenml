...
training_pipeline.add_preprocesser(
    ImagePreprocessor(
        image_key="image", labels=["label"], shape=[-1, 28, 28, 3]
    )
)

# Add a trainer
training_pipeline.add_trainer(
    ImageTensorflowTrainer(
...
training_pipeline.add_evaluator(
    TFMAEvaluator(
        slices=[[naming_utils.transformed_label_name("label")]],
        metrics={
            naming_utils.transformed_label_name("label"): ["binary_accuracy"]
        },
    )
)
