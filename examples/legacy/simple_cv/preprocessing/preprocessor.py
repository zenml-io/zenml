...
from zenml.steps.preprocesser.standard_preprocesser.methods.methods_nonseq_filling import (
    _impute,
)
...
def decode_image(input_):
    ...
    image = tf.map_fn(
        lambda x: tf.io.decode_image(x[0], channels=3), densed, dtype=tf.uint8
    )
    return image
...
class ImagePreprocessor(BasePreprocesserStep):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            shape=shape, image_key=image_key, labels=labels, **kwargs
        )
