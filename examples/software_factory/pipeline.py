...
def software_factory(
    ...
    if needs_final_tests:
        # The last action was a `fix`, so the previous test report is stale
        # for the branch's current HEAD. Re-run tests so the deploy
        # approval metadata reflects the actual state being deployed.
        tests = run_tests(
            workspace=workspace,
            repo=repo,
            branch=target_branch,
