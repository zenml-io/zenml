...
class ZenMLSandboxEnvironment(BaseEnvironment):
    ...
    async def start(self, force_build: bool) -> None:
        ...
        # B2b spike patch (branch spike/b2b-trl-harbor only, do not ship):
        # Harbor >=0.9 deprecated allow_internet for network_mode (default
        # "public") and parses the old field to None, so the 0.8-era
        # `if not cfg.allow_internet` refused EVERY task on current
        # Harbor. Refuse only when isolation is actually requested,
        # whichever field expresses it.
        network_mode = getattr(cfg, "network_mode", None)
        if network_mode is not None:
            requires_isolation = network_mode.value != "public"
        else:
            requires_isolation = cfg.allow_internet is False
        if requires_isolation:
            raise NotImplementedError(
                "This Harbor task requests network isolation "
                f"(network_mode={network_mode!r}, "
                f"allow_internet={cfg.allow_internet!r}), but the ZenML "
                "Sandbox bridge cannot enforce network isolation yet — "
                "refusing to run rather than silently skip it."
            )
        ignored = [
            name
            for name, value in (
