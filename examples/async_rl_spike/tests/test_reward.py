...
Each case invokes sandbox_scripts/score_pipeline.py the way the rollout
scorer does, as a subprocess, so the test exercises the real scoring path,
...
def score(tmp_path: Path, completion: str, spec: dict) -> dict:
    """Run the scorer exactly as the rollout scorer does inside the sandbox.
