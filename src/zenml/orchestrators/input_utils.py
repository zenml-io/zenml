...
def resolve_step_inputs(
    ...
    input_overrides: Optional[Dict[str, "InputSource"]] = None,
) -> Tuple[
    ...
    sources: Dict[str, Tuple["InputSource", bool]] = {
        name: (source, False) for name, source in step.config.inputs.items()
    }
    sources.update(
        {name: (source, True) for name, source in input_overrides.items()}
    )

    for name, (source, overridden) in sources.items():
...
def _resolve_input_source(
    ...
    try:
        ...
        if (
            lazy_loader.artifact_name is None
            and lazy_loader.metadata_name
        ...
        elif lazy_loader.artifact_name is None:
            err_msg = (
