...
from tfx.orchestration.portable.mlmd import common_utils, event_lib
from tfx.proto.orchestration import execution_result_pb2, pipeline_pb2
from tfx.types import artifact_utils
from tfx.utils import proto_utils, typing_utils
...
def is_execution_successful(execution: metadata_store_pb2.Execution) -> bool:
    ...
    return (
        ...
        or execution.last_known_state == metadata_store_pb2.Execution.CACHED
    )
...
def is_step_successful(step: StepRunModel) -> bool:
    return step.status in {ExecutionStatus.COMPLETED, ExecutionStatus.CACHED}


def is_step_running(step: StepRunModel) -> bool:
...
def is_execution_active(execution: metadata_store_pb2.Execution) -> bool:
    ...
    """
    return (
        execution.last_known_state == metadata_store_pb2.Execution.NEW
        or execution.last_known_state == metadata_store_pb2.Execution.RUNNING
    )
...
def sort_steps_newest_to_oldest(
    steps: List[StepRunModel],
) -> List[StepRunModel]:
    return sorted(steps, key=lambda s: s.created, reverse=True)
...
def prepare_execution(
    ...
    execution.type_id = common_utils.register_type_if_not_exist(
        metadata_handler, execution_type
    ).id
    if execution_name:
        ...
        for index, artifact in enumerate(artifact_list):
            if (
                ...
                if artifact.mlmd_artifact.HasField("id"):
                    artifact_event_map[artifact.id] = (
...
def get_executions_associated_with_all_contexts(
    ...
    for context in contexts:
        executions = metadata_handler.store.get_executions_by_context(
            context.id
        )
        if executions_dict is None:
            ...
            executions_dict = {
                e.id: e for e in executions if e.id in executions_dict
            }
...
def get_artifacts_dict(
    ...
    artifacts_by_id = {
        a.id: artifact_utils.deserialize_artifact(
            artifact_types_by_id[a.type_id], a
        )
        for a in mlmd_artifacts
    }
...
def set_execution_result(
    ...
    execution.custom_properties[
        _EXECUTION_RESULT
    ].string_value = json_format.MessageToJson(execution_result)
