...
def is_valid_output_event(
    event: metadata_store_pb2.Event, expected_output_key: Optional[str] = None
) -> bool:
    ...
    if expected_output_key:
        return (
            len(event.path.steps) == 2
            and event.path.steps[0].key  # Valid event should have 2 steps.
            == expected_output_key
...
def is_valid_input_event(
    event: metadata_store_pb2.Event, expected_input_key: Optional[str] = None
) -> bool:
...
def add_event_path(
    event: metadata_store_pb2.Event, key: str, index: int
) -> None:
...
def get_artifact_path(event: metadata_store_pb2.Event) -> Tuple[str, int]:
    ...
    if len(event.path.steps) != 2:
        raise ValueError(
            "Expected exactly two steps corresponding to key and index in event: {}".format(
                event
            )
        )
