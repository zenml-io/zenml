...
class BaseExecutorOperator(abc.ABC):
    ...
    def __init__(
        ...
        """
        if not isinstance(
            executor_spec, tuple(t for t in self.SUPPORTED_EXECUTOR_SPEC_TYPE)
        ):
            raise RuntimeError(
                "Executor spec not supported: %s" % executor_spec
            )
        if platform_config and not isinstance(
            platform_config,
            tuple(t for t in self.SUPPORTED_PLATFORM_CONFIG_TYPE),
        ):
            raise RuntimeError(
                "Platform spec not supported: %s" % platform_config
            )
    ...
    def with_execution_watcher(
        self, execution_watcher_address: str
