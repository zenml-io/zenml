...
DEFAULT_EXECUTOR_OPERATORS = {
    executable_spec_pb2.PythonClassExecutableSpec: python_executor_operator.PythonExecutorOperator,
    executable_spec_pb2.BeamExecutableSpec: beam_executor_operator.BeamExecutorOperator,
    executable_spec_pb2.ContainerExecutableSpec: docker_executor_operator.DockerExecutorOperator,
}

DEFAULT_DRIVER_OPERATORS = {
    executable_spec_pb2.PythonClassExecutableSpec: python_driver_operator.PythonDriverOperator
}

# LINT.IfChange
_SYSTEM_NODE_HANDLERS = {
    "tfx.dsl.components.common.importer.Importer": importer_node_handler.ImporterNodeHandler,
    "tfx.dsl.components.common.resolver.Resolver": resolver_node_handler.ResolverNodeHandler,
}
...
class _ExecutionFailedError(Exception):
    ...
    def __init__(
        self, err_msg: str, executor_output: execution_result_pb2.ExecutorOutput
    ):
...
class Launcher:
    ...
    def __init__(
        ...
        custom_driver_operators: Optional[
            Dict[Any, Type[DriverOperator]]
        ] = None,
    ):
        ...
        if executor_spec:
            self._executor_operator = self._executor_operators[
                type(executor_spec)
        ...
        if custom_driver_spec:
            self._driver_operator = self._driver_operators[
                type(custom_driver_spec)
    ...
    def _register_or_reuse_execution(
        ...
        if len(executions) > 1:
            ...
            if execution_lib.is_execution_successful(execution):
                logging.warning(
                    "This execution has already succeeded. Will exit "
    ...
    def _prepare_execution(
        ...
        with self._mlmd_connection as m:
            ...
            if execution_lib.is_execution_successful(execution):
                # RMTFX: not necessary, the execution will be newly created
        ...
        if self._driver_operator:
            # RMTFX: not necessary, driver not used
        ...
        with self._mlmd_connection as m:
            ...
            # RMTFX: check if cached step exists in DB
            ...
            if (
                self._pipeline_node.execution_options.caching_options.enable_cache
            ):
                ...
                if cached_outputs is not None:
                    ...
                    logging.info(
                        "An cached execusion %d is used.", execution.id
                    )
                    return (
                        _ExecutionPreparationResult(
                            execution_info=self._build_execution_info(
            ...
            return (
                _ExecutionPreparationResult(
                    execution_info=self._build_execution_info(
                        ...
                        exec_properties=exec_properties,
                        execution_output_uri=(
                            self._output_resolver.get_executor_output_uri(
                                execution.id
                            )
                        ),
                        ...
                        tmp_dir=self._output_resolver.make_tmp_dir(
                            execution.id
                        ),
                    ),
                    execution_metadata=execution,
    ...
    def _update_with_driver_output(
        ...
        for key, artifact_list in driver_output.output_artifacts.items():
            ...
            assert all(
                artifact_cls == a.type for a in output_dict[key][1:]
    ...
    def launch(self) -> Optional[data_types.ExecutionInfo]:
        ...
        execution_preparation_result, step_run_id = self._prepare_execution()
        ...
        if is_execution_needed:
            ...
            try:
                ...
                execution_output = (
                    e.executor_output
                    if isinstance(e, _ExecutionFailedError)
                    else None
                )
                ...
                self._publish_failed_execution(
                    execution_info.execution_id, contexts, execution_output
                )
                logging.error(
                    "Execution %d failed.", execution_info.execution_id
                )
                raise
            finally:
            ...
            logging.info(
                "Publishing output artifacts %s for execution %s",
                execution_info.output_dict,
            ...
            for name, artifact_list in execution_info.output_dict.items():
                artifact = artifact_list[0]
                properties = (
                    executor_output.output_artifacts["output"]
                    .artifacts[0]
                    .properties
                )
                a = ArtifactModel(
                    ...
                    producer_step_id=step_run.id,
                    type=artifact.artifact_type.name,
                    uri=artifact.uri,
                    materializer=properties["materializer"].string_value,
                    data_type=properties["datatype"].string_value,
                    is_cached=False,
                )
            ...
            self._publish_successful_execution(
                execution_info.execution_id,
                contexts,
                execution_info.output_dict,
                executor_output,
