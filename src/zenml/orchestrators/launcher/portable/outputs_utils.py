...
def _attach_artifact_properties(
    spec: pipeline_pb2.OutputSpec.ArtifactSpec, artifact: types.Artifact
):
    ...
    for key, value in spec.additional_properties.items():
        if not value.HasField("field_value"):
            raise RuntimeError(
                "Property value is not a field_value for %s" % key
            )
        setattr(
            artifact,
            key,
    ...
    for key, value in spec.additional_custom_properties.items():
        ...
        if value_type == "int_value":
            ...
            artifact.set_string_custom_property(
                key, value.field_value.string_value
            )
        elif value_type == "double_value":
            artifact.set_float_custom_property(
                key, value.field_value.double_value
            )
        else:
...
class OutputsResolver:
    ...
    def __init__(
        ...
        self._node_dir = os.path.join(
            self._pipeline_root, pipeline_node.node_info.id
        )
        ...
        for key, output_spec in self._pipeline_node.outputs.outputs.items():
            ...
            if isinstance(artifact, ValueArtifact):
                artifact.uri = os.path.join(
                    artifact.uri, _VALUE_ARTIFACT_FILE_NAME
                )
    ...
    def get_executor_output_uri(self, execution_id: int) -> str:
        ...
        execution_dir = os.path.join(
            self._node_dir, _SYSTEM, _EXECUTOR_EXECUTION, str(execution_id)
        )
    ...
    def get_stateful_working_directory(
        ...
        if (
            ...
            and self._execution_mode != pipeline_pb2.Pipeline.SYNC
        ):
            raise ValueError(
        ...
        stateful_working_dir = os.path.join(
            self._node_dir, _SYSTEM, _STATEFUL_WORKING_DIR, dir_suffix
        )
        try:
            fileio.makedirs(stateful_working_dir)
        except Exception:  # pylint: disable=broad-except
            logging.exception(
                "Failed to make stateful working dir: %s", stateful_working_dir
            )
            raise
