...
from tfx.orchestration.portable.input_resolution import exceptions, processor
from tfx.orchestration.portable.mlmd import event_lib, execution_lib
...
def get_qualified_artifacts(
    ...
    try:
        ...
        logging.warning(
            "Artifact type %s is not found in MLMD.", artifact_type.name
        )
        artifact_type = None
    ...
    qualified_output_events = [
        ev
        for ev in metadata_handler.store.get_events_by_execution_ids(
            qualified_producer_executions
        )
...
def _resolve_single_channel(
    metadata_handler: metadata.Metadata, channel: pipeline_pb2.InputSpec.Channel
) -> List[types.Artifact]:
    ...
    for context_query in channel.context_queries:
        context = metadata_handler.store.get_context_by_type_and_name(
            ...
            data_types_utils.get_value(context_query.name),
        )
        if context:
...
def _is_sufficient(
    artifact_multimap: Mapping[str, Sequence[types.Artifact]],
    ...
    *,
    ...
    try:
        ...
        raise exceptions.InputResolutionError(
            f"Error occurred during input resolution: {str(e)}."
        ) from e
...
def resolve_parameters(
    ...
    for key, value in node_parameters.parameters.items():
        ...
        result[key] = getattr(
            value.field_value, value.field_value.WhichOneof("value")
        )
