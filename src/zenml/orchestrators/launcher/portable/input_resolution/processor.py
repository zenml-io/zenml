...
import inspect
from typing import Iterable, Sequence, Type, Union, cast
...
from tfx.utils import json_utils, name_utils, typing_utils
...
def _run_resolver_strategy(
    ...
    *,
    ...
    if not typing_utils.is_artifact_multimap(input_dict):
        raise TypeError(
            f"Invalid argument type: {input_dict!r}. Must be "
    ...
    *,
    ...
    for step in resolver_steps:
        ...
        if issubclass(cls, resolver.ResolverStrategy):
            ...
            raise TypeError(
                f"Invalid class {cls}. Should be a subclass of "
