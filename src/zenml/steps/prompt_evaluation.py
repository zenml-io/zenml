...
def llm_judge_evaluate_prompt(
    ...
    """
    logger.info(f"Starting LLM-as-Judge evaluation for prompt {prompt_artifact_id}")
    ...
    try:
        ...
        if not prompt_content:
            raise ValueError(f"No prompt template found in artifact {prompt_artifact_id}")
        ...
        for i, test_case in enumerate(test_cases):
            ...
            filled_prompt = _fill_prompt_template(prompt_content, test_case.get("variables", {}))
            ...
            evaluation_results.append({
                ...
                "scores": case_scores,
                "overall_score": sum(case_scores.values()) / len(case_scores)
            })
        ...
        average_scores = {criterion: total / num_cases for criterion, total in total_scores.items()}
        ...
        evaluation_summary = {
            ...
            "recommendations": _generate_recommendations(average_scores, evaluation_results)
        }
        
        logger.info(f"Evaluation complete. Overall score: {overall_score:.2f} ({quality_level})")
...
def compare_prompt_versions(
    ...
    logger.info(f"Comparison complete. Winner: {winner} with {improvement:.1f}% improvement")
...
def _generate_recommendations(scores: Dict[str, float], results: List[Dict]) -> List[str]:
    ...
    if weakest_criterion[1] < 6.0:
        recommendations.append(f"Focus on improving {weakest_criterion[0]} (current score: {weakest_criterion[1]:.1f})")
    
    # Check for consistency issues
    score_variance = max(scores.values()) - min(scores.values())
    if score_variance > 2.0:
        recommendations.append("Consider balancing the prompt to address inconsistent performance across criteria")
    
    # Overall score recommendations
    avg_score = sum(scores.values()) / len(scores)
    if avg_score < 7.0:
        recommendations.append("Overall prompt quality could be improved with more specific instructions")
    
    return recommendations


def _create_detailed_comparison(eval_v1: Dict, eval_v2: Dict) -> Dict[str, Any]:
    ...
    for criterion in v1_scores.keys():
        ...
        comparison[criterion] = {
            ...
            "winner": "v2" if score_diff > 0 else "v1" if score_diff < 0 else "tie"
        }
