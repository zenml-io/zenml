...
def compare_prompts(prompt1: Prompt, prompt2: Prompt) -> PromptComparison:
    ...
    from zenml.prompts.prompt_comparison import (
        compare_prompts as _compare_prompts,
    )
...
def find_best_prompt(
    prompts: List[Prompt], metric: str = "accuracy", prefer_recent: bool = True
) -> Optional[Prompt]:
...
def validate_prompt_template(template: str) -> Tuple[bool, List[str]]:
    ...
    if template.count("{{") > template.count("{") - template.count("{{"):
        issues.append(
            "Template may contain escaped braces that won't be replaced"
        )
...
def generate_prompt_report(prompts: List[Prompt]) -> str:
    ...
    report.append(
        f"- Unique Tasks: {len(set(tasks))} ({', '.join(set(tasks))})"
    )
    report.append(
        f"- Unique Domains: {len(set(domains))} ({', '.join(set(domains))})"
    )
    report.append(
        f"- Prompt Types: {len(set(prompt_types))} ({', '.join(set(prompt_types))})"
    )
    report.append("")
    ...
    if prompts_with_performance:
        ...
        report.append(
            f"- Prompts with Performance Data: {len(prompts_with_performance)}"
        )
    ...
    for i, prompt in enumerate(prompts, 1):
        report.append(
            f"### {i}. {prompt.description or f'Prompt {prompt.prompt_id[:8]}...'}"
        )
        ...
        if prompt.performance_metrics:
            metrics_str = ", ".join(
                f"{k}={v:.3f}" for k, v in prompt.performance_metrics.items()
            )
