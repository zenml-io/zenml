...
class TemplateComparison(BaseModel):
    ...
    common_variables: List[str] = Field(default_factory=list, description="Variables present in both templates")
    unique_to_first: List[str] = Field(default_factory=list, description="Variables only in first template")
    unique_to_second: List[str] = Field(default_factory=list, description="Variables only in second template")
    diff_lines: List[str] = Field(default_factory=list, description="Line-by-line diff")
    character_diff: List[Tuple[str, str]] = Field(default_factory=list, description="Character-level differences")
...
class PromptComparison(BaseModel):
    ...
    is_compatible: bool = Field(..., description="Whether prompts are compatible for A/B testing")
    
    # Field-by-field differences
    differences: List[PromptDiff] = Field(default_factory=list, description="List of differences between prompts")
    
    # Template comparison
    template_comparison: TemplateComparison = Field(..., description="Detailed template comparison")
    ...
    # Metadata
    compared_at: datetime = Field(default_factory=datetime.now, description="When comparison was performed")
    comparison_version: str = Field(default="1.0", description="Version of comparison algorithm")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "major_differences": [
                diff for diff in self.differences 
                if diff.field in ["template", "task", "domain", "prompt_strategy"]
            ],
        }
...
def compare_prompts(prompt1: Prompt, prompt2: Prompt) -> PromptComparison:
    ...
    # Compare templates
    template_comparison = _compare_templates(prompt1.template, prompt2.template)
    
    # Calculate overall similarity
    overall_similarity = _calculate_overall_similarity(prompt1, prompt2, template_comparison)
...
def _calculate_differences(prompt1: Prompt, prompt2: Prompt) -> List[PromptDiff]:
    """Calculate field-by-field differences between prompts."""
    differences = []
    
    # Fields to compare
    fields_to_compare = [
        "template", "prompt_type", "task", "domain", "prompt_strategy",
        "instructions", "context_template", "model_config_params",
        "target_models", "description", "version", "tags", "variables",
        "examples", "min_tokens", "max_tokens", "expected_format",
        "language", "safety_checks"
    ]
...
def _compare_templates(template1: str, template2: str) -> TemplateComparison:
    ...
    diff_lines = list(difflib.unified_diff(
        lines1, lines2, 
        fromfile="prompt1", tofile="prompt2", 
        lineterm=""
    ))
...
def _calculate_overall_similarity(
    ...
    similarity_score += weights["template"] * template_comparison.similarity_score
    ...
    if prompt1.task == prompt2.task:
        ...
        if pair in related_tasks or pair[::-1] in related_tasks:
            similarity_score += weights["task"] * related_tasks.get(pair, related_tasks.get(pair[::-1], 0))
    ...
    if prompt1.target_models == prompt2.target_models:
        ...
        overlap = len(set(prompt1.target_models).intersection(set(prompt2.target_models)))
        total = len(set(prompt1.target_models).union(set(prompt2.target_models)))
...
def _check_compatibility(
    ...
    if prompt1.domain and prompt2.domain:
        ...
        if domain_pair in incompatible_domains or domain_pair[::-1] in incompatible_domains:
            return False
...
def generate_comparison_report(comparison: PromptComparison) -> str:
    ...
    report.append(f"Compatible for A/B Testing: {'Yes' if comparison.is_compatible else 'No'}")
    report.append("")
    ...
    report.append(f"- Template similarity: {summary['template_similarity']:.2%}")
    ...
    report.append(f"- Common variables: {', '.join(template.common_variables) if template.common_variables else 'None'}")
    
    if template.unique_to_first:
        report.append(f"- Variables only in Prompt 1: {', '.join(template.unique_to_first)}")
    if template.unique_to_second:
        report.append(f"- Variables only in Prompt 2: {', '.join(template.unique_to_second)}")
    ...
    if comparison.performance_comparison:
        ...
        for metric, data in comparison.performance_comparison.items():
            ...
            report.append(f"- **{metric}**: {data['prompt1']:.3f} → {data['prompt2']:.3f} ({symbol} {abs(improvement):.1f}%)")
