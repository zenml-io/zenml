...
def api_token(
    ...
    * Workload API token: This token is scoped to a specific pipeline run, step
    ...
    Generic API tokens can only be generated by users authenticated via the
    dashboard.

    Workload API tokens can only be generated for clients that are authenticated
    with an authorized device or a service account and API key and the client
    must be authorized to create pipeline runs. A workload API token can be
    used to authenticate and generate another workload API token, but only for
    the same schedule, pipeline run ID or step run ID, in that order.
    ...
    if token_type == APITokenType.GENERIC:
        ...
        if token.device_id or token.api_key_id:
            raise AuthorizationException(
                "Generic API tokens can only be generated for users "
                "authenticated via the ZenML dashboard."
            )
    ...
    if (
        not token.device_id
        and not token.api_key_id
        and not token.schedule_id
        and not token.pipeline_run_id
        and not token.step_run_id
    ):
        raise AuthorizationException(
            "Workload API tokens can only be generated by clients "
            "authenticated with an authorized device, a service account API "
            "key or another workload API token."
        )
