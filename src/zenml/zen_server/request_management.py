...
class RequestManager:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # allowed to keep running after the client receives a timeout response:
        # retries for those requests are tied to the same API transaction.
        # Non-deduplicated requests keep the connection and operation lifetime
        # aligned to avoid hidden duplicate mutations on client retry.
        try:
            timeout: Optional[float] = None
            if deduplicate:
                # We take into account the time that has already elapsed since
                # the request was received to avoid keeping the request for too
                # long.
                timeout = max(
