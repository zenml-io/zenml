...
from zenml.integrations.seldon_v2 import SELDON_V2_MODEL_DEPLOYER_FLAVOR
...
if TYPE_CHECKING:
    from zenml.integrations.seldon_v2.model_deployers import (
        SeldonV2ModelDeployer,
    )


class SeldonV2ModelDeployerConfig(BaseModelDeployerConfig):
    """Config for the Seldon V2 Model Deployer.

    Attributes:
        kubernetes_context: the Kubernetes context to use to contact the remote
            Seldon Core V2 installation. If not specified, the current
            configuration is used. Depending on where the Seldon V2 model deployer
        ...
        kubernetes_namespace: the Kubernetes namespace where the Seldon Core V2
            ...
            Depending on where the Seldon V2 model deployer is being used, this can
        ...
        base_url: the base URL of the Kubernetes ingress used to expose the
            Seldon Core V2 deployment servers.
        secret: the name of a ZenML secret containing the credentials used by
            Seldon Core V2 storage initializers to authenticate to the Artifact
        ...
        kubernetes_secret_name: the name of the Kubernetes secret containing
            the credentials used by Seldon Core V2 storage initializers to
...
class SeldonV2ModelDeployerFlavor(BaseModelDeployerFlavor):
    """Seldon Core V2 model deployer flavor."""
    ...
    def name(self) -> str:
        ...
        """
        return SELDON_V2_MODEL_DEPLOYER_FLAVOR
    ...
    @property
    def config_class(self) -> Type[SeldonV2ModelDeployerConfig]:
        """Returns `SeldonV2ModelDeployerConfig` config class.

        Returns:
                The config class.
        """
        return SeldonV2ModelDeployerConfig

    @property
    def implementation_class(self) -> Type["SeldonV2ModelDeployer"]:
