...
def seldon_custom_model_deployer_step(
    ...
    Raises:
        ValueError: if the custom deployer is not defined or if the predict
            function path is not valid
        TypeError: if the predict function path is not a callable function
...
def seldon_mlflow_registry_deployer_step(
    ...
    Raises:
        ValueError: if registry_model_name is not provided or if neither
            registry_model_version nor registry_model_stage is provided or if
            the MLflow experiment tracker is not available in the active stack
