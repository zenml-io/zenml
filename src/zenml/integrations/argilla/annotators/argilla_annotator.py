...
from argilla_sdk import Client, Project
...
from zenml.integrations.argilla.steps.argilla_standard_steps import (
    ArgillaDatasetSyncParameters,
)
...
class ArgillaAnnotator(BaseAnnotator, AuthenticationMixin):
    ...
    def _connection_available(self) -> bool:
        ...
        # TODO: [HIGH] refactor to use a more specific exception
            ...
            logger.error(
                "Connection error: No connection was able to be established to the Argilla backend."
            )
            return False
    ...
    def _get_azure_import_storage_sources(
        ...
        Raises:
            ConnectionError: If the connection to the Argilla backend is unavailable.
        """
    ...
    def _storage_source_already_exists(
        self,
        uri: str,
        params: ArgillaDatasetSyncParameters,
        dataset: Project,
    ) -> bool:
    ...
    def get_parsed_label_config(self, dataset_id: int) -> Dict[str, Any]:
        """Returns the parsed Argilla label config for a dataset.
    ...
    def populate_artifact_store_parameters(
        ...
        if artifact_store.flavor == "s3":
            ...
            raise RuntimeError(
                ...
                "store. The Argilla annotator needs explicit credentials "
            ...
            if gcp_credentials:
                ...
                # Argilla will need to read it from a file
                secret_folder = Path(
            ...
            if azure_credentials:
                ...
                raise RuntimeError(
                    "The Argilla annotator could not use the "
        ...
        raise RuntimeError(
            ...
            "supported by ZenML's Argilla integration. "
        ...
        if params.storage_type == "azure":
            if not params.azure_account_name or not params.azure_account_key:
                logger.warning(
                    ...
                    "settings in the Argilla web UI as per your needs."
                )
            ...
            if not params.google_application_credentials:
                logger.warning(
                    ...
                    "synchronization settings in the Argilla web UI as "
                    "per your needs."
                )
            ...
            if (
                ...
                logger.warning(
                    ...
                    "Argilla web UI as per your needs."
                )
            ...
            del os.environ["ARGILLA_LOCAL_FILES_SERVING_ENABLED"]
            del os.environ["ARGILLA_LOCAL_FILES_DOCUMENT_ROOT"]
        else:
            raise ValueError(
                f"Invalid storage type. '{params.storage_type}' is not "
                "supported by ZenML's Argilla integration. Please choose "
