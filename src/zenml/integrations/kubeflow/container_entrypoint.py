...
import sys
import textwrap
from typing import cast, Dict, List, Mapping, MutableMapping, Optional, \
    Sequence, Tuple, Union, Any
...
def _get_grpc_metadata_connection_config(
  ...
  """
    ...
    connection_config.host = os.getenv(grpc_config["grpc_service_host"]["environment_variable"])
    connection_config.port = int(os.getenv(grpc_config["grpc_service_port"]["environment_variable"]))
    return connection_config
  ...
  """
    ...
    for name, spec in node.outputs.outputs.items():
        if spec.artifact_spec.type.name == standard_artifacts.ModelRun.TYPE_NAME:
            ...
            tensorboard_output = {'type': 'tensorboard',
...
def main(argv):
    ...
    metadata_connection_config = _get_grpc_metadata_connection_config(kubeflow_metadata_config)
    ...
    with telemetry_utils.scoped_labels({
        ...
        Repository().set_active_stack("local_stack")
        ...
        setattr(main_module, executor_class_name, getattr(step_module, executor_class_name))
