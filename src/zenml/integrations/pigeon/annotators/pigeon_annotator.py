...
"""Implementation of the Pigeon annotation integration."""
...
class LabelStudioAnnotator(BaseAnnotator, AuthenticationMixin):
    """Class to interact with the Pigeon annotation interface."""
    ...
    def _get_client(self) -> Client:
        ...
        Raises:
            ValueError: when unable to access the Pigeon API key.
        """
        secret = self.get_authentication_secret()
        if not secret:
            raise ValueError(
                "Unable to access predefined secret to access Pigeon API key."
            )
        api_key = secret.secret_values.get("api_key")
        if not api_key:
            raise ValueError(
                "Unable to access Pigeon API key from secret."
            )
    ...
    def _connection_available(self) -> bool:
        ...
        # TODO: [HIGH] refactor to use a more specific exception
            ...
            logger.error(
                "Connection error: No connection was able to be established to the Pigeon backend."
            )
            return False
    ...
    def add_dataset(self, **kwargs: Any) -> Any:
        ...
        Args:
            **kwargs: Additional keyword arguments to pass to the Pigeon client.

        Returns:
            A Pigeon Project object.
    ...
    def delete_dataset(self, **kwargs: Any) -> None:
        ...
        Args:
            **kwargs: Additional keyword arguments to pass to the Pigeon
                client.
        ...
        if not dataset_id:
            raise ValueError(
                f"Dataset name '{dataset_name}' has no corresponding `dataset_id` in Pigeon."
            )
    ...
    def _get_azure_import_storage_sources(
        ...
        Raises:
            ConnectionError: If the connection to the Pigeon backend is unavailable.
        """
    ...
    def get_parsed_label_config(self, dataset_id: int) -> Dict[str, Any]:
        """Returns the parsed Pigeon label config for a dataset.
    ...
    def populate_artifact_store_parameters(
        ...
        if artifact_store.flavor == "s3":
            ...
            raise RuntimeError(
                ...
                "store. The Pigeon annotator needs explicit credentials "
            ...
            if gcp_credentials:
                ...
                # Pigeon will need to read it from a file
                secret_folder = Path(
            ...
            if azure_credentials:
                ...
                raise RuntimeError(
                    "The Pigeon annotator could not use the "
        ...
        raise RuntimeError(
            ...
            "supported by ZenML's Pigeon integration. "
        ...
        if params.storage_type == "azure":
            if not params.azure_account_name or not params.azure_account_key:
                logger.warning(
                    ...
                    "settings in the Pigeon web UI as per your needs."
                )
            ...
            if not params.google_application_credentials:
                logger.warning(
                    ...
                    "synchronization settings in the Pigeon web UI as "
                    "per your needs."
                )
            ...
            if (
                ...
                logger.warning(
                    ...
                    "Pigeon web UI as per your needs."
                )
            ...
            # Set the environment variables required by Pigeon
            ...
            raise ValueError(
                ...
                "supported by ZenML's Pigeon integration. Please choose "
