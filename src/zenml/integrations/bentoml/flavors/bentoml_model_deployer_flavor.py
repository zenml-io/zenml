...
if TYPE_CHECKING:
    from zenml.integrations.kserve.model_deployers import BentoMLModelDeployer
...
class BentoMLModelDeployerConfig(BaseModelDeployerConfig):
    ...
    Attributes:
        kubernetes_context: the Kubernetes context to use to contact the remote
            BentoML installation. If not specified, the current
            configuration is used. Depending on where the BentoML model deployer
        ...
        kubernetes_namespace: the Kubernetes namespace where the BentoML
            ...
            Depending on where the BentoML model deployer is being used, this can
