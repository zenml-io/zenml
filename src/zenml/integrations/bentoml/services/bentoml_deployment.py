...
from bentoml.yatai import YataiClient, get_yatai_client
...
class BentoMLDeploymentConfig(ServiceConfig):
    ...
    Attributes:
        ...
        predictor: the BentoML predictor used to serve the model.
    ...
    def get_kubernetes_labels(self) -> Dict[str, str]:
        """Generate the labels for the BentoML inference CRD from the service configuration.

        These labels are attached to the BentoML inference service CRD
        and may be used as label selectors in lookup operations.

        Returns:
            The labels for the BentoML inference service CRD.
        """
        labels = {"app": "zenml"}
    ...
    def get_kubernetes_annotations(self) -> Dict[str, str]:
        """Generate the annotations for the BentoML inference CRD the service configuration.

        The annotations are used to store additional information about the
        BentoML ZenML service associated with the deployment that is
        ...
        recreate the service configuration from a remote BentoML inference
        service CRD.

        Returns:
            The annotations for the BentoML inference service CRD.
        """
        annotations = {
    ...
    def create_from_deployment(
        ...
        """Recreate a BentoML service from a BentoML deployment resource.

        Args:
            deployment: the BentoML inference service CRD.

        Returns:
            The BentoML ZenML service configuration corresponding to the given
            BentoML inference service CRD.

        Raises:
            ValueError: if the given deployment resource does not contain
                the expected annotations or it contains an invalid or
                incompatible BentoML ZenML service configuration.
        """
        ...
        try:
            ...
            raise ValueError(
                f"The loaded BentoML Inference Service resource contains an "
                f"invalid or incompatible BentoML ZenML service configuration: "
                f"{config_data}"
            ) from e
        return service_config


class BentoMLDeploymentService(BaseService):
    """A ZenML service that represents a BentoML inference service CRD.
    ...
    SERVICE_TYPE = ServiceType(
        ...
        type="model-serving",
        flavor="kserve",
        description="BentoML inference service",
    )

    config: BentoMLDeploymentConfig = Field(
        default_factory=BentoMLDeploymentConfig
    )
    status: ServiceStatus = Field(default_factory=ServiceStatus)

    def _get_model_deployer(self) -> "BentoMLModelDeployer":
        """Get the active BentoML model deployer.

        Returns:
            The active BentoMLModelDeployer.

        Raises:
            TypeError: if the current stack has no BentoMLModelDeployer.
        """
        ...
        try:
            ...
            raise TypeError(
                "No active BentoML model deployer is present in the active "
                "stack. Please make sure that a BentoML model deployer is "
                "present in the active stack."
            )
        return model_deployer

    def _get_client(self) -> YataiClient:
        """Get the BentoML client from the active BentoML model deployer.
    ...
    def _get_namespace(self) -> Optional[str]:
        """Get the Kubernetes namespace from the active BentoML model deployer.
    ...
    def check_status(self) -> Tuple[ServiceState, str]:
        """Check the state of the BentoML inference service.

        This method Checks the current operational state of the external BentoML
    ...
    def crd_name(self) -> str:
        """Get the name of the BentoML inference service CRD that uniquely corresponds to this service instance.

        Returns:
            The name of the BentoML inference service CRD.
        """
        return (
    ...
    def _get_kubernetes_labels(self) -> Dict[str, str]:
        """Generate the labels for the BentoML inference service CRD from the service configuration.

        Returns:
            The labels for the BentoML inference service.
        """
        labels = self.config.get_kubernetes_labels()
        labels["zenml.service_uuid"] = str(self.uuid)
        BentoMLDeploymentConfig.sanitize_labels(labels)
        return labels

    @classmethod
    def create_from_deployment(
        cls, deployment: bentoml.Service
    ) -> "BentoMLDeploymentService":
        """Recreate the configuration of a BentoML Service from a deployed instance.

        Args:
            deployment: the BentoML deployment resource.

        Returns:
            The BentoML service configuration corresponding to the given
            BentoML deployment resource.

        Raises:
            ValueError: if the given deployment resource does not contain
                the expected annotations or it contains an invalid or
                incompatible BentoML service configuration.
        """
        config = BentoMLDeploymentConfig.create_from_deployment(deployment)
    ...
    def provision(self) -> None:
        """Provision or update remote BentoML deployment instance.
        ...
        if self.config.container is not None:
            ...
            predictor_kwargs = {
                self.config.predictor: bentoml.Service(
                    storage_uri=self.config.model_uri,
                    resources=self.config.resources,
                )
            }

        isvc = bentoml.Service(
            ...
            spec=bentoml.Service(
                predictor=bentoml.Service(**predictor_kwargs)
            ),
        )

        # TODO[HIGH]: better error handling when provisioning BentoML instances
        try:
    ...
    def deprovision(self, force: bool = False) -> None:
        ...
        # TODO[HIGH]: catch errors if deleting a BentoML instance that is no
        #   longer available
        try:
            client.delete(name=name, namespace=namespace)
        except RuntimeError:
            raise ValueError(
                f"Could not delete BentoML instance '{name}' from namespace: '{namespace}'."
            )
    ...
    def _get_deployment_logs(
        ...
        """Get the logs of a BentoML deployment resource.

        Args:
            name: the name of the BentoML deployment to get logs for.
        ...
        try:
            ...
            if not pods:
                raise Exception(
                    f"The BentoML deployment {name} is not currently "
    ...
    def get_logs(
        ...
        """Retrieve the logs from the remote BentoML inference service instance.
