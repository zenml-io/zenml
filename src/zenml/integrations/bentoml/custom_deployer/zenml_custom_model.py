...
"""Implements a custom model for the BentoML integration."""
...
class ZenMLCustomModel(bentoml.Service, bentoml.Runnable):  # type: ignore[misc]
    """Custom model class for ZenML and BentoMl.

    This class is used to implement a custom model for the BentoML integration,
    ...
    def init_bentoml_service(self, runners: List) -> bentoml.Service:
        """
        Creates and return the BentoMl Service Object 

        Arguments:
            runners {List} -- ML Model 
        """
        return bentoml.Service(self.model_name, runners=runners)

    def predict(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """Predict the given request.

        The main predict function of the model. This function is called by the
        BentoML server when a request is received. Then inside this function,
...
@click.option(
    ...
    required=True,
    type=click.STRING,
    help="The name of the model to deploy. This is important for the BentoML server.",
)
@click.option(
    "--predict_func",
    required=True,
...
def main(model_name: str, model_uri: str, predict_func: str) -> None:
    """Main function responsible for starting the BentoML server.

    The way the custom deployment server works with the BentoML server is by
    implementing a custom model class and passing it to the BentoML server and then
    ...
    entrypoint_command = [
        "python",
        "-m",
        "zenml.integrations.BentoML.custom_deployer.zenml_custom_model",
        "--model_name",
