...
class EvidentlyNumericalTargetDriftDetectionConfig(BaseDriftDetectionConfig):
    """Config class for the Evidently numerical target drift detection step.

    column_mapping: properties of the dataframe's columns used for numerical
...
class EvidentlyNumericalTargetDriftDetectionStep(BaseDriftDetectionStep):
    """Simple step implementation which implements Evidently's functionality for
    detecting numerical target data drift."""

    def entrypoint(  # type: ignore[override]
        self,
        reference_dataset: DataArtifact,
        comparison_dataset: DataArtifact,
        config: EvidentlyNumericalTargetDriftDetectionConfig,
        context: StepContext,
    ) -> dict:  # type: ignore[type-arg]
        """Main entrypoint for the Evidently numerical target drift detection
        step.
        ...
        """
        data_drift_profile = Profile(sections=[NumTargetDriftProfileSection()])
