...
class EvidentlyCategoricalTargetDriftDetectionConfig(BaseDriftDetectionConfig):
    """Config class for the Evidently categorical target drift detection step.

    column_mapping: properties of the dataframe's columns used for categorical
...
class EvidentlyCategoricalTargetDriftDetectionStep(BaseDriftDetectionStep):
    """Simple step implementation which implements Evidently's functionality for
    detecting categorical target data drift."""

    def entrypoint(  # type: ignore[override]
        self,
        reference_dataset: DataArtifact,
        comparison_dataset: DataArtifact,
        config: EvidentlyCategoricalTargetDriftDetectionConfig,
        context: StepContext,
        ...
        """
        data_drift_profile = Profile(sections=[CatTargetDriftProfileSection()])
