...
"""Utils for the local Tekton deployment behaviors."""
...
def tekton_pipelines_ready(kubernetes_context: str) -> bool:
    """Returns whether all Tekton Pipelines pods are ready.
    ...
    Returns:
        Whether all Tekton Pipelines pods are ready.
    """
    try:
        subprocess.check_call(
            [
                ...
                "pods",
                "-l",
                "application-crd-id=tekton-pipelines",
            ],
...
def wait_until_tekton_pipelines_ready(kubernetes_context: str) -> None:
    """Waits until all Tekton Pipelines pods are ready.
    ...
    """
    logger.info(
        "Waiting for all Tekton Pipelines pods to be ready (this might "
        "take a few minutes)."
    )
    while True:
        ...
        if tekton_pipelines_ready(kubernetes_context=kubernetes_context):
            break
...
def deploy_tekton_pipelines(kubernetes_context: str) -> None:
    """Deploys Tekton Pipelines.

    Args:
        kubernetes_context: The kubernetes context on which Tekton Pipelines
            should be deployed.
    """
    logger.info("Deploying Tekton Pipelines.")
    subprocess.check_call(
        [
            "kubectl",
            "--context",
            ...
            "apply",
            "-k",
            f"github.com/tekton/pipelines/manifests/kustomize/cluster-scoped-resources?ref={KFP_VERSION}&timeout=5m",
        ]
    )
    subprocess.check_call(
        [
    ...
    subprocess.check_call(
        [
            ...
            "apply",
            "-k",
            f"github.com/tekton/pipelines/manifests/kustomize/env/platform-agnostic-pns?ref={KFP_VERSION}&timeout=5m",
        ]
    )

    wait_until_tekton_pipelines_ready(kubernetes_context=kubernetes_context)
    logger.info("Finished Tekton Pipelines setup.")


def add_hostpath_to_tekton_pipelines(
    kubernetes_context: str, local_path: str
) -> None:
    """Patches the Tekton Pipelines deployment to mount a local folder.

    This folder serves as a hostpath for visualization purposes.

    This function reconfigures the Tekton pipelines deployment to use a
    ...
    https://github.com/tekton/pipelines/blob/master/docs/config/volume-support.md
    ...
    """
    logger.info("Patching Tekton Pipelines to mount a local folder.")

    pod_template = {
        "spec": {
            "serviceAccountName": "tekton-pipelines-viewer",
            "containers": [
                {
                    "volumeMounts": [
                        {
    ...
    logger.debug(
        "Adding host path volume for local path `%s` to tekton pipeline"
    ...
    logger.debug(
        "Adding host path volume for local path `%s` to the tekton UI",
        local_path,
    )
    subprocess.check_call(
        [
...
def start_tekton_ui_daemon(
    ...
    This is so the Tekton Pipelines UI is accessible in the browser.
    ...
    if not networking_utils.port_available(port):
        ...
        logger.warning(
            "Unable to port-forward Tekton Pipelines UI to local port %d "
        ...
        logger.warning(
            ...
            "In order to access the Tekton Pipelines UI at "
        ...
        def _daemon_function() -> None:
            """Port-forwards the Tekton Pipelines UI pod."""
        ...
        logger.info(
            "Started Tekton Pipelines UI daemon (check the daemon logs at %s "
            "in case you're not able to view the UI). The Tekton Pipelines "
...
def stop_tekton_ui_daemon(pid_file_path: str) -> None:
    ...
    if fileio.exists(pid_file_path):
        if sys.platform == "win32":
            ...
            logger.info("Stopped Tekton Pipelines UI daemon.")
