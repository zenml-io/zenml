...
# inspired by the tekton dag runner implementation of tfx
...
class TektonOrchestrator(BaseOrchestrator):
    ...
    def _get_k3d_cluster_name(uuid: UUID) -> str:
        ...
        return f"zenml-tekton-{str(uuid)[:8]}"
    ...
    def _get_k3d_kubernetes_context(uuid: UUID) -> str:
        ...
        """
        return f"k3d-{TektonOrchestrator._get_k3d_cluster_name(uuid)}"
        ...
        def _validate_local_requirements(stack: "Stack") -> Tuple[bool, str]:
            ...
            if self.kubernetes_context not in contexts:
                ...
                logger.warning(
                    ...
                    f"configured for the Tekton orchestrator is not the "
                    ...
                    f"configured in the Tekton orchestrator and silence "
                    f"this warning, run:\n\n"
            ...
            if not self.skip_local_validations and self.is_local:
                ...
                if not container_registry.is_local:
                    return False, (
                        ...
                        f"The local Tekton orchestrator only works with a "
    ...
    def get_docker_image_name(self, pipeline_name: str) -> str:
        ...
        """
        base_image_name = f"zenml-tekton:{pipeline_name}"
    ...
    def prepare_pipeline_deployment(
        ...
        logger.debug("Tekton docker container requirements: %s", requirements)
    ...
    def _configure_container_op(container_op: dsl.ContainerOp) -> None:
        ...
        # security context:
        # https://www.tekton.org/docs/components/pipelines/installation/localcluster-deployment/#deploying-tekton-pipelines
        ...
        # status (https://www.tekton.org/docs/components/pipelines/installation/choose-executor/#emissary-executor)
    ...
    def prepare_or_run_pipeline(
        ...
        def _construct_kfp_pipeline() -> None:
            ...
            for step in sorted_steps:
                ...
                # Create a container_op - the tekton equivalent of a step. It
    ...
    def _get_k3d_registry_name(self, port: int) -> str:
        ...
        Returns:
            The registry name.
        """
        return f"k3d-zenml-tekton-registry.localhost:{port}"
    ...
    def is_daemon_running(self) -> bool:
        """Returns if the local Tekton UI daemon for this orchestrator is running.
    ...
    def resume(self) -> None:
        ...
        """
        if self.is_running:
            logger.info("Local tekton pipelines deployment already running.")
            return

        if not self.is_provisioned:
            raise ProvisioningError(
                "Unable to resume local tekton pipelines deployment: No "
    ...
    def suspend(self) -> None:
        ...
        if not self.is_provisioned:
            logger.info("Local tekton pipelines deployment not provisioned.")
            return
