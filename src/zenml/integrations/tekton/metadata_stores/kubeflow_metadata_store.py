...
"""Implementation of the Tekton metadata store."""
...
from zenml.integrations.tekton import KUBEFLOW_METADATA_STORE_FLAVOR
from zenml.integrations.tekton.orchestrators.tekton_orchestrator import (
    TektonOrchestrator,
)
...
class TektonMetadataStore(BaseMetadataStore):
    """Tekton GRPC backend for ZenML metadata store."""
    ...
    def validator(self) -> Optional[StackValidator]:
        ...
        def _ensure_kfp_orchestrator(stack: Stack) -> Tuple[bool, str]:
            return (
                ...
                "The Tekton metadata store can only be used with a Tekton "
                "orchestrator.",
            )
    ...
    def get_tfx_metadata_config(
        ...
        """Return tfx metadata config for the tekton metadata store.

        Returns:
            The tfx metadata config for the tekton metadata store.
    ...
    @property
    def kfp_orchestrator(self) -> TektonOrchestrator:
        """Returns the Tekton orchestrator in the active stack.

        Returns:
            The Tekton orchestrator in the active stack.
        """
        repo = Repository(skip_repository_check=True)  # type: ignore[call-arg]
        return cast(TektonOrchestrator, repo.active_stack.orchestrator)

    @property
    def kubernetes_context(self) -> str:
        """Returns the kubernetes context.

        This is returned to the cluster where the Tekton Pipelines services
        are running.
    ...
    def suspend(self) -> None:
        ...
        if not self.is_running:
            logger.info("Local tekton pipelines deployment not running.")
            return
    ...
    def start_kfp_metadata_daemon(self) -> None:
        ...
        This is so the Tekton Pipelines Metadata MySQL database is accessible
        on the localhost.
        ...
        if sys.platform == "win32":
            logger.warning(
                ...
                "In order to access the Tekton Pipelines Metadata locally, "
            ...
            raise ProvisioningError(
                f"Unable to port-forward Tekton Pipelines Metadata to local "
                f"port {self.port} because the port is occupied. In order to "
                f"access the Tekton Pipelines Metadata locally, please "
            ...
            def _daemon_function() -> None:
                """Forwards the port of the Tekton Pipelines Metadata pod ."""
            ...
            logger.info(
                "Started Tekton Pipelines Metadata daemon (check the daemon"
    ...
    def wait_until_metadata_store_ready(
        ...
        """
        logger.info(
            "Waiting for the Tekton metadata store to be ready (this might "
            "take a few minutes)."
        )
        while True:
            try:
                ...
                logger.info(
                    "The Tekton metadata store is not ready yet. Waiting for "
                    "10 seconds..."
                )
                if timeout <= 0:
                    raise RuntimeError(
                        f"An unexpected error was encountered while waiting for the "
                        f"Tekton metadata store to be functional: {str(e)}"
                    ) from e
                timeout -= 10
                time.sleep(10)

        logger.info("The Tekton metadata store is functional.")
