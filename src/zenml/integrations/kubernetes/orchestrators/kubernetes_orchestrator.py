...
class KubernetesOrchestrator(ContainerizedOrchestrator):
    ...
    def prepare_or_run_pipeline(
        ...
        if settings.synchronous:
            ...
            kube_utils.wait_pod(
                # We pass the k8s client through a lambda instead of directly to
                # allow the client to be refreshed if the connector has expired.
                core_api=lambda: self._k8s_core_api,
                pod_name=pod_name,
                namespace=self.config.kubernetes_namespace,
