...
class ZenMLSagemakerSession(sagemaker.Session):
    """Sagemaker session linked to a ZenML service connector.

    This is a workaround for the fact that the native Sagemaker session does not
    support refreshing the session credentials when they expire (in our case,
    the temporary credentials generated by the AWS service connector associated
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initializes a `ZenMLSagemakerSession` instance.

        Args:
            stack_component: The stack component that is linked to the AWS
                service connector that will be used to initialize and refresh
                the Sagemaker session credentials.
            *args: Additional arguments to pass to the `sagemaker.Session`
                constructor.
            **kwargs: Additional keyword arguments to pass to the
                `sagemaker.Session` constructor.
        """
        self.stack_component = stack_component
        self._connector_session = None
        super().__init__(boto_session=self._get_boto_session(), *args, **kwargs)

    @property
    def boto_session(self) -> boto3.Session:
        """Returns a boto session.
        ...
        # Guaranteed that the connector is not None, otherwise this class
    ...
    @boto_session.setter
    def boto_session(self, boto_session: boto3.Session) -> None:
        """Sets the boto session.

        Args:
            boto_session: The boto session.
        """
        # We don't want to set the boto session directly, but instead use the
        # connector to manage it.
        pass
