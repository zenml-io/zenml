...
"""Implementation of the Argo entrypoint configuration."""
...
class ArgoEntrypointConfiguration(StepEntrypointConfiguration):
    """Entrypoint configuration for running steps on Argo."""
    ...
    def get_custom_entrypoint_arguments(
        ...
        """
        # Argo replaces the `$(context.pipelineRun.name)` with the actual
        # run name when executing a container. This allows users to re-trigger
        # runs on the Argo UI and uses the new run name for storing
