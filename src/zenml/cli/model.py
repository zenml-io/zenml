...
def register_model_deployer_subcommands() -> None:
    ...
    if model_deployer_group:
        ...
        def describe_model(
            ...
            if served_models:
                print_served_model_configuration(
                    served_models[0], model_deployer
                )
                return
            warning(
        ...
        def get_url(
            ...
            if served_models:
                try:
                    ...
                    prediction_hostname = (
                        model_deployer.get_model_server_info(
                            served_models[0]
                        ).get("PREDICTION_HOSTNAME")
                    ...
                    warning(
                        "The deployed model instance has no 'prediction_url'."
                    )
                return
            warning(
                f"No model with uuid: '{served_model_uuid}' could be found."
