...
from zenml.cli.utils import warn_unsupported_non_default_workspace
...
def list_workspaces() -> None:
    ...
    workspaces = Client().zen_store.list_workspaces()
    ...
    if workspaces:
        ...
        cli_utils.declare("No workspaces registered.")
...
def describe_workspace(workspace_name_or_id: Optional[str] = None) -> None:
    ...
    if not workspace_name_or_id:
        ...
        workspace = client.get_workspace(workspace_name_or_id)
        cli_utils.print_pydantic_models(
            [workspace], exclude_columns=["created", "updated"]
        )
...
def set_workspace(workspace_name_or_id: str) -> None:
    ...
    cli_utils.print_active_config()
    try:
        Client().set_active_workspace(workspace_name_or_id=workspace_name_or_id)
...
def delete_workspace(workspace_name_or_id: str) -> None:
    ...
    if not confirmation:
        cli_utils.declare("Workspace deletion canceled.")
        return
    try:
