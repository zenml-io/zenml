...
T = TypeVar("T", bound=IdentifiedEntityResponse)  # type: ignore[type-arg]
...
def print_pydantic_models(
    ...
    def __dictify(model: T) -> Dict[str, str]:
        ...
        if not columns:
            if isinstance(model, IdentifiedEntityResponse):
                include_columns = ["id"]
        ...
        for k in include_columns:
            ...
            if isinstance(value, IdentifiedEntityResponse):
            ...
            #  the list and extract either name or id
                ...
                for v in value:
                    if isinstance(v, IdentifiedEntityResponse):
