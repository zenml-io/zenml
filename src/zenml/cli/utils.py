...
T = TypeVar("T", bound=BaseDBResponse)  # type: ignore[type-arg]
...
def print_pydantic_models(
    ...
    def __dictify(model: T) -> Dict[str, str]:
        ...
        if not columns:
            if isinstance(model, BaseDBResponse):
                include_columns = ["id"]
        ...
        for k in include_columns:
            ...
            if isinstance(value, BaseDBResponse):
            ...
            #  the list and extract either name or id
                ...
                for v in value:
                    if isinstance(v, BaseDBResponse):
