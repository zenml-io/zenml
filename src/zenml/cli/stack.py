...
def _get_stack_component_info(
    ...
    if component_type == "artifact_store":
        ...
        def query_region(
            provider: StackDeploymentProvider, compute_type: str
        ) -> str:
            region = Prompt.ask(
                f"Select the location for your {compute_type}:",
                choices=sorted(
                    Client()
                    .zen_store.get_stack_deployment_info(provider)
                    .locations.values()
                ),
                show_choices=True,
            )
        ...
        if flavor == "sagemaker":
            ...
            config["region"] = query_region(
                StackDeploymentProvider.GCP, "Skypilot cluster"
            )
        elif flavor == "vm_azure":
            config["region"] = query_region(
                StackDeploymentProvider.AZURE, "Skypilot cluster"
            )
        elif flavor == "vertex":
