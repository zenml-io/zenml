...
ZENML_PROJECT_TEMPLATES = dict(
    e2e_batch=ZenMLProjectTemplateLocation(
        ...
        github_tag="2025.12.17",  # Make sure it is aligned with .github/workflows/update-templates-to-examples.yml
    ),
...
def go() -> None:
    ...
    Raises:
        ...
        subprocess.CalledProcessError: If Jupyter Notebook fails to launch.
    """
    from zenml.cli.text_utils import (
        zenml_cli_privacy_message,
        zenml_cli_welcome_message,
...
@cli.command(
    "backfill-database-compression",
    help="Compress legacy execution-definition database payloads.",
    hidden=True,
)
@click.option(
    "--batch-size",
    ...
    default=250,
    show_default=True,
    help="Maximum rows processed in each database transaction.",
)
@click.option(
    "--apply",
    "apply_changes",
    is_flag=True,
    default=False,
    help="Persist the compression backfill. Without this flag, only report.",
)
def backfill_database_compression(
    batch_size: int, apply_changes: bool
) -> None:
    """Compress existing execution-definition payloads.

    Args:
        batch_size: Maximum rows processed per transaction.
        apply_changes: Whether to persist the compressed representations.
    """
    from zenml.zen_stores.base_zen_store import BaseZenStore
    ...
    if store_config.type != StoreType.SQL:
        cli_utils.warning(
            "Unable to backfill database compression while connected to a "
            "ZenML server."
        )
        return
    ...
    results = store.backfill_execution_definition_compression(
        batch_size=batch_size,
        apply=apply_changes,
    )

    for result in results:
        action = "updated" if apply_changes else "compressible"
        cli_utils.declare(
            f"{result.table_name}.{result.column_name}: "
            f"scanned={result.scanned_rows}, "
            f"{action}={result.updated_rows if apply_changes else result.compressible_rows}, "
            f"logical_bytes_saved={result.bytes_saved}"
        )

    total_saved = sum(result.bytes_saved for result in results)
    if apply_changes:
        cli_utils.declare(
            f"Compression backfill finished. Logical bytes saved: "
            f"{total_saved}."
        )
    else:
        cli_utils.declare(
            f"Dry run finished. Potential logical bytes saved: "
            f"{total_saved}. Re-run with --apply only after all server "
            "replicas support compressed execution definitions."
        )
