...
class RestZenStoreConfiguration(StoreConfiguration):
    ...
    @validator("api_token")
    def validate_api_token(cls, api_token: Optional[str]) -> Optional[str]:
        """Validates that either api_token or username is set.

        Args:
            api_token: The api_token to be validated.

        Raises:
            ValueError: If neither api_token nor username is set.

        Returns:
            The validated api_token.
        """
        if api_token is not None:
            return api_token
        if api_token is None and cls.username is None:
            raise ValueError("neither api_token nor username is set")
        return api_token
...
class RestZenStore(BaseZenStore):
    ...
    def api_token(self) -> str:
        """The authentication token for the REST store.

        If the token is not set, it will be retrieved from the config or
        ...
        if self._api_token is None:
            self._api_token = self.config.api_token or self._get_auth_token()
