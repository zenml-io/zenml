...
class SourceOrObject:
    ...
    Attributes:
        ...
        _object: Loaded object object (if initialized from callable
            or loaded).
    ...
    def __init__(
        ...
        Raises:
            ValueError: If value is not a string, Source, or object.
        """
    ...
    def load(self) -> ObjectType:
        ...
        if not self._is_loaded:
            ...
            try:
                self._object = load(self._source)
                self._is_loaded = True
    ...
    def to_source_string(self) -> str:
        """Convert to source import path string.
...
def _validate_source_or_object(value: Any) -> SourceOrObject:
    ...
    """
    if isinstance(value, SourceOrObject):
        return value
    return SourceOrObject(value)


def _serialize_source_or_object(value: SourceOrObject) -> str:
...
SourceOrObjectField = Annotated[
    SourceOrObject,
    BeforeValidator(_validate_source_or_object),
    PlainSerializer(_serialize_source_or_object, return_type=str),
]
