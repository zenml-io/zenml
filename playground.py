...
class ZenMLPipeline:
    ...
    @abstractmethod
    def get_tfx_pipeline(self):
        component_list = []
        for s in self.steps:
            # create the component list somehow
            component_list.append(s.to_component())

        return component_list


class MyPipeline(ZenMLPipeline):
    def __init__(self):
...
class SplitStep(ZenMLBaseStep):
    ...
    def process(self, input_artifact, output_artifact):
...
class DistributedBaseStep(ZenMLBaseStep):
    def __init__(self):
        super(DistributedBaseStep, self).__init__()
