...
PREPENDED_SECRETS = [
    PrependedSecret(combined_key_name='aria_cat_age',
                    group_name='aria',
                    expected_key='cat_age'),  # standard
    PrependedSecret(combined_key_name='axel__cat_age',
                    group_name='axel',
                    expected_key='_cat_age'),  # key with leading _
    PrependedSecret(combined_key_name='kube_kube_',
                    group_name='kube',
                    expected_key='kube_'),  # key same as group
    PrependedSecret(combined_key_name='kube_kube_kube',
                    group_name='kube',
                    expected_key='kube_kube'),  # key contains group
    PrependedSecret(combined_key_name='square__square',
                    group_name='square_',
                    expected_key='square'),  # group with trailing _
    PrependedSecret(combined_key_name='missing_group',
                    group_name='rand',
                    potential_error=RuntimeError),  # group not in combined key
    PrependedSecret(combined_key_name='missing_key_',
                    group_name='missing_key_',
                    potential_error=RuntimeError),  # no key in combined key
    PrependedSecret(combined_key_name='',
                    group_name='',
                    potential_error=RuntimeError),  # empty inputs
    PrependedSecret(combined_key_name='',
                    group_name='g',
                    potential_error=RuntimeError),  # empty combined key
    PrependedSecret(combined_key_name='g',
                    group_name='',
                    potential_error=RuntimeError),  # empty group
]
