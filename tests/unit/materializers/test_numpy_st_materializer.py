...
from zenml.materializers.numpy_st_materializer import NumpySTMaterializer
from zenml.metadata.metadata_types import (
    DType,
)


def test_numpy_st_materializer():
    """Test the numpy materializer using safetensors with metadata extraction."""

    numeric_array = np.array([1, 2, 3, -1, 0.4])
    ...
    numeric_result, numeric_metadata = _test_materializer(
        ...
        materializer_class=NumpySTMaterializer,
    ...
    assert numeric_metadata["shape"] == (5,)
    assert numeric_metadata["dtype"] == DType(numeric_array.dtype.type)
    assert numeric_metadata["mean"] == 1.08
    assert numeric_metadata["std"] == 1.3658696863171098
