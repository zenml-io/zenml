...
def test_defaultdict_with_integer_keys_materialization():
    """Test that defaultdicts load as regular dicts."""
...
def test_counter_with_integer_keys_materialization():
    """Test materialization for a counter with integer keys."""
    with TemporaryDirectory(
        dir=Client().active_stack.artifact_store.path
    ) as artifact_uri:
        materializer = BuiltInContainerMaterializer(uri=artifact_uri)
        materializer.save(Counter({1: 10}))

        result = materializer.load(Counter)

    assert type(result) is Counter
    assert result == Counter({1: 10})
