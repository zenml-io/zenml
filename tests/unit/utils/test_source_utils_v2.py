...
def test_basic_source_loading():
    ...
    assert source_utils_v2.load("zenml.client.Client") is client.Client
    client_source = Source(
        module="zenml.client", attribute="Client", type=SourceType.UNKNOWN
    )
    assert source_utils_v2.load(client_source) is client.Client

    client_module_source = Source(
        module="zenml.client", attribute=None, type=SourceType.INTERNAL
    )
    assert source_utils_v2.load(client_module_source) is client

    with pytest.raises(ModuleNotFoundError):
        source_utils_v2.load("zenml.not_a_module.Class")

    with pytest.raises(AttributeError):
        source_utils_v2.load("zenml.client.NotAClass")
...
def test_user_source_loading_prepends_source_root(mocker, tmp_path):
    ...
    with pytest.raises(ModuleNotFoundError):
        source_utils_v2.load(wrong_source)
    ...
    assert source_utils_v2.load(correct_source) == 1
...
def test_basic_source_resolving(mocker):
    ...
    assert source_utils_v2.resolve(current_module) == Source(
        module=expected_module_name, attribute=None, type=SourceType.USER
    )
    assert source_utils_v2.resolve(EmptyClass) == Source(
        module=expected_module_name,
        attribute=EmptyClass.__name__,
        type=SourceType.USER,
    )
    assert source_utils_v2.resolve(empty_function) == Source(
        module=expected_module_name,
        attribute=empty_function.__name__,
        type=SourceType.USER,
    )

    assert source_utils_v2.resolve(int) == Source(
        module=int.__module__, attribute=int.__name__, type=SourceType.BUILTIN
    )
    assert source_utils_v2.resolve(defaultdict) == Source(
        module=defaultdict.__module__,
        attribute=defaultdict.__name__,
        type=SourceType.BUILTIN,
    )
    assert source_utils_v2.resolve(source_utils_v2) == Source(
        module=source_utils_v2.__name__,
        attribute=None,
        type=SourceType.INTERNAL,
    )
    assert source_utils_v2.resolve(pytest) == Source(
...
def test_source_resolving_fails_for_non_toplevel_classes_and_functions(mocker):
    ...
    with pytest.raises(RuntimeError):
        source_utils_v2.resolve(EmptyClass.NestedClass)

    with pytest.raises(RuntimeError):
        source_utils_v2.resolve(inline_function)
...
def test_module_type_detection(mocker):
    ...
    assert (
        source_utils_v2.get_source_type(builtin_module) == SourceType.BUILTIN
    )

    standard_lib_module = sys.modules[defaultdict.__module__]
    assert (
        source_utils_v2.get_source_type(standard_lib_module)
        == SourceType.BUILTIN
    )
    assert source_utils_v2.is_standard_lib_file(standard_lib_module.__file__)

    internal_module = sys.modules[source_utils_v2.__name__]
    assert (
        source_utils_v2.get_source_type(internal_module) == SourceType.INTERNAL
    )
    assert source_utils_v2.is_internal_module(internal_module.__name__)

    distribution_package_module = sys.modules[pytest.__name__]
    assert (
        source_utils_v2.get_source_type(distribution_package_module)
        == SourceType.DISTRIBUTION_PACKAGE
    )
    assert source_utils_v2.is_distribution_package_file(
    ...
    assert source_utils_v2.get_source_type(user_module) == SourceType.USER
    assert source_utils_v2.is_user_file(user_module.__file__)
...
def test_prepend_python_path():
    ...
    with source_utils_v2.prepend_python_path(path):
...
def test_setting_a_custom_source_root():
    ...
    initial_source_root = source_utils_v2.get_source_root()
    source_utils_v2.set_custom_source_root(source_root="custom_source_root")
    assert source_utils_v2.get_source_root() == "custom_source_root"
    source_utils_v2.set_custom_source_root(source_root=None)
    assert source_utils_v2.get_source_root() == initial_source_root
...
def test_validating_source_classes(mocker):
    ...
    assert not source_utils_v2.validate_source_class(
        instance_source, expected_class=EmptyClass
    )
    ...
    assert source_utils_v2.validate_source_class(
        class_source, expected_class=EmptyClass
    )
...
def test_package_utility_functions():
    ...
    assert (
        source_utils_v2._get_package_for_module(
            module_name=ExitCode.__module__
        )
        == "pytest"
    )
    assert (
        source_utils_v2._get_package_version(package_name="pytest")
        == pytest.__version__
    )
    ...
    assert (
        source_utils_v2._get_package_version(
