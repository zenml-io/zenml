...
def test_default_artifact_name(one_step_pipeline):
...
def test_custom_artifact_name(one_step_pipeline):
...
def test_multi_output_artifact_names(one_step_pipeline):
...
def test_artifact_versioning(one_step_pipeline):
...
def test_artifact_versioning_duplication(one_step_pipeline):
...
def test_artifact_tagging(one_step_pipeline):
...
def test_artifact_step_run_linkage(one_step_pipeline):
...
def test_load_artifact_visualization(one_step_pipeline):
    ...
    for i in range(len(artifact.visualizations)):
        load_artifact_visualization(
            artifact=artifact, index=i, zen_store=client.zen_store
        )
...
def _get_output_of_last_run() -> ArtifactVersionResponse:
    """Get the output of the last run."""
    return list(Client().list_pipeline_runs()[0].steps.values())[0].output
...
def _get_metadata_of_last_run() -> Dict[str, "RunMetadataResponse"]:
    """Get the artifact metadata of the last run."""
    return _get_output_of_last_run().run_metadata
