...
@step(model_config=ModelVersionProducerConfig(name=MODEL_NAME))
...
def test_link_multiple_named_outputs_with_self_context_and_caching():
    ...
    with model_killer():
        ...
        m1 = ModelVersionConsumerConfig(
            name=MODEL_NAME,
        ).get_or_create_model()
        m2 = ModelVersionConsumerConfig(
            name="bar",
        ).get_or_create_model()
...
@step(model_config=ModelVersionConsumerConfig(name="step", version="step"))
...
@pipeline(
    enable_cache=False,
    model_config=ModelVersionConsumerConfig(name="pipe", version="pipe"),
)
...
@step(
    model_config=ModelVersionConsumerConfig(
        name=MODEL_NAME, version="good_one"
    )
)
...
def test_link_no_versioning():
    ...
    with model_killer():
        ...
        # manual creation needed, as we work with specific versions
        model = ModelVersionConsumerConfig(
            name=MODEL_NAME,
        ).get_or_create_model()
...
@pipeline(
    enable_cache=False,
    model_config=ModelVersionConsumerConfig(name=MODEL_NAME),
)
...
def test_artifacts_linked_from_cache_steps():
    ...
    with model_killer():
        ...
        for i in range(1, 3):
            fake_version = ModelVersionProducerConfig(
                name="bar"
