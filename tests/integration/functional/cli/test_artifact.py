...
def test_artifact_list(client_with_run):
...
def test_artifact_version_list(client_with_run):
...
def test_artifact_update(client_with_run):
    ...
    artifact_response = client_with_run.list_artifacts()[0]
    ...
    assert result.exit_code == 0
    artifact_response = client_with_run.get_artifact(new_name)
    assert artifact_response.name == new_name
    for tag in tags:
...
def test_artifact_version_update(client_with_run):
    ...
    # Initially there should be no tags
    artifact_version_response = client_with_run.list_artifact_versions()[0]
    for tag in tags:
    ...
    assert result.exit_code == 0
    artifact_version_response = client_with_run.get_artifact_version(
...
def test_artifact_prune(client_with_run):
    """Test that `zenml artifact prune` deletes all unused artifacts."""

    # Initially there should be two artifacts with one version each
    existing_artifacts = client_with_run.list_artifacts()
    assert len(existing_artifacts) == 2
    existing_artifact_versions = client_with_run.list_artifact_versions()
    ...
    client_with_run.delete_pipeline_run(run_name)
    existing_runs = client_with_run.list_runs()
