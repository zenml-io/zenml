...
def test_update_stack_update_on_default_fails() -> None:
    """Test stack update of default stack is prohibited."""
    client = Client()
    # first we set the active stack to a non-default stack
    original_stack = client.active_stack_model

    new_artifact_store = _create_local_artifact_store(client)
...
def test_update_stack_active_stack_succeeds() -> None:
    """Test stack update of active stack succeeds."""
    client = Client()
    # first we set the active stack to a non-default stack
    registered_stack = client.active_stack_model
    ...
    client.activate_stack(stack_name_id_or_prefix=new_stack.id)
...
def test_updating_non_active_stack_succeeds() -> None:
    ...
    orchestrator = _create_local_orchestrator(client)

    new_orchestrator = client.create_stack_component(
    ...
    assert (
        client.get_stack(str(new_stack.id))
...
def test_update_stack_adding_component_succeeds() -> None:
    ...
    client.activate_stack(new_stack.id)

    local_image_builder = _create_local_image_builder(client)

    local_image_builder_model = client.create_stack_component(
    ...
    new_stack = client.get_stack(new_stack.id)
...
def test_update_stack_adding_to_default_stack_fails() -> None:
...
def test_update_stack_nonexistent_stack_fails() -> None:
    client = Client()
    ...
    assert result.exit_code == 1
    assert client.active_stack.image_builder is None


def test_rename_stack_nonexistent_stack_fails() -> None:
...
def test_rename_stack_new_name_with_existing_name_fails() -> None:
    runner = CliRunner()
    rename_command = cli.commands["stack"].commands["rename"]
...
def test_rename_stack_default_stack_fails() -> None:
    ...
    assert result.exit_code == 1
    assert len(client.list_stacks(name="default")) == 1


def test_rename_stack_active_stack_succeeds() -> None:
    ...
    assert result.exit_code == 0
    assert client.active_stack_model.name == "axls_stack"


def test_rename_stack_non_active_stack_succeeds() -> None:
    ...
    result = runner.invoke(rename_command, ["arias_stack", "axls_stack"])
    assert result.exit_code == 0
    assert client.get_stack(new_stack.id).name == "axls_stack"


def test_remove_component_from_nonexistent_stack_fails() -> None:
...
def test_remove_component_core_component_fails() -> None:
    """Test stack remove-component of core component fails."""
    client = Client()
    # first we create a non-default stack
    new_artifact_store = _create_local_artifact_store(client)

    new_artifact_store_model = client.create_stack_component(
        name=new_artifact_store.name,
        flavor=new_artifact_store.flavor,
        component_type=new_artifact_store.type,
        configuration=new_artifact_store.config.model_dump(),
    ...
    new_orchestrator = _create_local_orchestrator(client)

    new_orchestrator_model = client.create_stack_component(
    ...
    arias_stack = client.get_stack(new_stack.name)
    assert StackComponentType.ORCHESTRATOR in arias_stack.components


def test_remove_component_non_core_component_succeeds() -> None:
    ...
    new_image_builder = _create_local_image_builder(client)

    new_image_builder_model = client.create_stack_component(
    ...
    assert (
        ...
        not in client.active_stack_model.components
    )


def test_delete_stack_with_flag_succeeds() -> None:
    ...
    with pytest.raises(KeyError):
        client.get_stack(new_stack.id)


def test_delete_stack_default_stack_fails() -> None:
    ...
    client.activate_stack(new_stack.name)

    runner = CliRunner()
    delete_command = cli.commands["stack"].commands["delete"]
    result = runner.invoke(delete_command, ["default", "-y"])
    assert result.exit_code == 1
    assert client.get_stack("default")


def test_delete_stack_recursively_with_flag_succeeds() -> None:
    ...
    assert client.get_stack_component(
        StackComponentType.ARTIFACT_STORE, artifact_store_name
    )
    assert client.get_stack_component(
        StackComponentType.ORCHESTRATOR, orchestrator_name
...
def test_stack_export_delete_import() -> None:
    ...
    new_stack_model = client.create_stack(
        name="arias_new_stack",
        components={
            StackComponentType.ARTIFACT_STORE: new_artifact_store_model.name,
            StackComponentType.ORCHESTRATOR: new_orchestrator_model.name,
    ...
    client.delete_stack(name_id_or_prefix=new_stack_model.name)
    ...
    with pytest.raises(KeyError):
        client.get_stack(new_stack_model.id)
    ...
    assert result.exit_code == 0
    assert client.get_stack(new_stack_model.name)


def test_stack_export_import_reuses_components() -> None:
    ...
    old_stack_model = client.create_stack(
        name=stack_name,
        components={
            StackComponentType.ARTIFACT_STORE: new_artifact_store_model.name,
            StackComponentType.ORCHESTRATOR: new_orchestrator_model.name,
    ...
    client.delete_stack(name_id_or_prefix=stack_name)

    # import stack
    import_command = cli.commands["stack"].commands["import"]
    result = runner.invoke(import_command, [stack_name])
    assert result.exit_code == 0
    new_stack_model = client.get_stack(stack_name)
