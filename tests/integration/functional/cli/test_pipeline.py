...
def test_pipeline_delete(clean_workspace_with_run: Client):
    """Test that zenml pipeline delete works as expected."""
    existing_pipelines = clean_workspace_with_run.list_pipelines()
    existing_deployments = clean_workspace_with_run.list_deployments()
    existing_runs = clean_workspace_with_run.list_pipeline_runs()
    ...
    updated_pipelines = clean_workspace_with_run.list_pipelines()
    assert len(updated_pipelines) == 0

    # Ensure pipeline deletion does not cascade pipeline runs or deployments
    updated_deployments = clean_workspace_with_run.list_deployments()
    assert len(updated_deployments) == len(existing_deployments)
    updated_runs = clean_workspace_with_run.list_pipeline_runs()
