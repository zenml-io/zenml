...
def pytest_collection_modifyitems(
    ...
    The medium tier runs many parallel sandboxes against a single shared
    external server (remote-mysql-modal), so tests that intentionally mutate
    global server state (global_state marker) would interfere with each other.
    Those tests run instead in the serial_only job or a dedicated lane.
    """
