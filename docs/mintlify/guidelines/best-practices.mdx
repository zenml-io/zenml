...
```
├── notebooks                   <- all notebooks in one place │ ├── *.ipynb pipelines <- training_pipeline .dockerignore config.yaml dockerfile training_pipeline.py requirements.txt deployment_pipeline ... steps loader_step loader_step.py training_step .gitignore readme.md run.py └── setup.sh< code></->
```
...
Cache is implicitly disabled for steps that have a [context](/advanced-guide/pipelines/step-metadata) argument, because it is assumed that you might use the step context to retrieve artifacts from the artifact store that are unrelated to the current step. However, if that is not the case, and your step logic doesn't invalidate the caching behavior, it would be better to explicitly enable the cache for your step.
...
When [inspecting pipeline runs](/starter-guide/pipelines) it is tempting to access the pipeline views directly by their index, but the pipelines within your `Repository` are sorted by time of first run, so the pipeline at `[-1]` might not be the one you are expecting.
