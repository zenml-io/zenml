...
<Frame caption="Sequence of events that happen when running a pipeline on a remote artifact store.
">
  <img src="/_assets/user-guide/production-guide/production-1.avif" />
</Frame>
...
Out of the box, ZenML ships with [many different supported artifact store flavors](/versions/0.66.0/stack-components/artifact-stores). For convenience, here are some brief instructions on how to quickly get up and running on the major cloud providers:
<Tabs>
  <Tab title="AWS">
...
The Amazon Web Services S3 Artifact Store flavor is provided by the [S3 ZenML integration](/versions/0.66.0/stack-components/artifact-stores/s3), you need to install it on your local machine to be able to register an S3 Artifact Store and add it to your stack:
...
```
For more information, read the [dedicated S3 artifact store flavor guide](/versions/0.66.0/stack-components/artifact-stores/s3).
  </Tab>
  <Tab title="CGP">
...
The Google Cloud Storage Artifact Store flavor is provided by the [GCP ZenML integration](/versions/0.66.0/stack-components/artifact-stores/gcp), you need to install it on your local machine to be able to register a GCS Artifact Store and add it to your stack:
...
```
For more information, read the [dedicated GCS artifact store flavor guide](/versions/0.66.0/stack-components/artifact-stores/gcp).
  </Tab>
  <Tab title="Azure">
...
The Microsoft Azure Artifact Store flavor is provided by the [Azure ZenML integration](/versions/0.66.0/stack-components/artifact-stores/azure), you need to install it on your local machine to be able to register an Azure Artifact Store and add it to your stack:
...
For more information, read the [dedicated Azure artifact store flavor guide](/versions/0.66.0/stack-components/artifact-stores/azure).
  </Tab>
<Tab title="Other">
You can create a remote artifact store in pretty much any environment, including other cloud providers using a cloud-agnostic artifact storage such as [Minio](/versions/0.66.0/stack-components/artifact-stores).

It is also relatively simple to create a [custom stack component flavor](/versions/0.66.0/how-to/stack-deployment/implement-a-custom-stack-component) for your use case.
...
While you can go ahead and [run your pipeline on your stack](/versions/0.66.0/user-guide/production-guide/remote-storage#running-a-pipeline-on-a-cloud-stack) if your local client is configured to access it, it is best practice to use a [service connector](/versions/0.66.0/how-to/auth-management) for this purpose. Service connectors are quite a complicated concept (We have a whole [docs section](/versions/0.66.0/how-to/auth-management) on them) - but we're going to be starting with a very basic approach.

First, let's understand what a service connector does. In simple words, a service connector contains credentials that grant stack components access to cloud infrastructure. These credentials are stored in the form of a [secret](/versions/0.66.0/how-to/interact-with-secrets), and are available to the ZenML server to use. Using these credentials, the service connector brokers a short-lived token and grants temporary permissions to the stack component to access that infrastructure. This diagram represents this process:
<Frame caption="Service Connectors abstract away complexity and implement security best practices
">
  <img src="/_assets/user-guide/production-guide/production-2.avif" />
</Frame>
<Tabs>
  <Tab title="AWS">
There are [many ways to create an AWS service connector](/versions/0.66.0/how-to/auth-management/aws-service-connector#authentication-methods), but for the sake of this guide, we recommend creating one by [using the IAM method](/versions/0.66.0/how-to/auth-management/aws-service-connector#aws-iam-role).
...
```
  ...
  <Tab title=" GCP">
There are [many ways to create a GCP service connector](/versions/0.66.0/how-to/auth-management/gcp-service-connector#authentication-methods), but for the sake of this guide, we recommend creating one by [using the Service Account method](/versions/0.66.0/how-to/auth-management/gcp-service-connector#gcp-service-account).
...
```
  ...
  <Tab title=" Azure">
There are [many ways to create an Azure service connector](/versions/0.66.0/how-to/auth-management/azure-service-connector#authentication-methods), but for the sake of this guide, we recommend creating one by [using the Service Principal method](/versions/0.66.0/how-to/auth-management/azure-service-connector#azure-service-principal).
...
Now that we have our remote artifact store registered, we can [register a new stack](/versions/0.66.0/user-guide/production-guide/understand-stacks#registering-a-stack) with it, just like we did in the previous chapter:
<Tabs>
  <Tab title="CLI">
```bash
...
```
  ...
  <Tab title=" Dashboard">
<Frame caption="Register a new stack.">
  <img src="/_assets/user-guide/production-guide/FCreateStack.png" />
</Frame>
  </Tab>
</Tabs>


Now, using the [code from the previous chapter](/versions/0.66.0/user-guide/production-guide/understand-stacks#run-a-pipeline-on-the-new-local-stack), we run a training pipeline:
...
When you run that pipeline, ZenML will automatically store the artifacts in the specified remote storage, ensuring that they are preserved and accessible for future runs and by your team members. You can ask your colleagues to connect to the same [ZenML server](/versions/0.66.0/user-guide/production-guide/deploying-zenml), and you will notice that if they run the same pipeline, the pipeline would be partially cached, **even if they have not run the pipeline themselves before**.
...
[ZenML Pro](https://zenml.io/pro) features an [Artifact Control Plane](/versions/0.66.0/user-guide/starter-guide/manage-artifacts) to visualize artifact versions:
<Frame caption="See artifact versions in the cloud.">
  <img src="/_assets/user-guide/production-guide/Fdcp_artifacts_versions_list.png" />
</Frame>
