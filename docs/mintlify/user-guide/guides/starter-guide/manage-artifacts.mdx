...
<Note>
Since custom versions cannot be duplicated, the above step can only be run once successfully. To avoid altering your code frequently, consider using a [YAML config](/versions/0.66.0/user-guide/production-guide/configure-pipeline) for artifact versioning.
</Note>
...
It is also common to consume an artifact downstream after producing it in an upstream pipeline or step. As we have learned in the [previous section](/versions/0.66.0/how-to/build-pipelines/fetching-pipelines#fetching-artifacts-directly), the `Client` can be used to fetch artifacts directly inside the pipeline code:
...
<Info>
Calls of `Client` methods like `get_artifact_version` directly inside the pipeline code makes use of ZenML's [late materialization](/versions/0.66.0/how-to/handle-data-artifacts/load-artifacts-into-memory) behind the scenes.
</Info>
...
One of the most useful ways of interacting with artifacts in ZenML is the ability to associate metadata with them. [As mentioned before](/versions/0.66.0/how-to/build-pipelines/fetching-pipelines#artifact-information), artifact metadata is an arbitrary dictionary of key-value pairs that are useful for understanding the nature of the data.
...
Additionally, there is a lot more to learn about artifacts within ZenML. Please read the [dedicated data management guide](/versions/0.66.0/how-to/handle-data-artifacts) for more information.
