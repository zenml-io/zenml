...
As discussed in the [Core Concepts](/getting-started/core-concepts), ZenML also contains the notion of a `Model`, which consists of many model versions (the iterations of the model). These concepts are exposed in the `Model Control Plane` (MCP for short).
...
The easiest way to use a ZenML model is to pass a `Model` object as part of a pipeline run. This can be done easily at a pipeline or a step level, or via a [YAML config](/user-guide/guides/production-guide/configure-pipeline).
...
When configured at the pipeline or step level, the model will be available through the [StepContext](/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-pipeline) or [PipelineContext](/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-pipeline).
...
[Just as one can associate metadata with artifacts](/user-guide/guides/starter-guide/manage-artifacts#logging-metadata-for-an-artifact), models too can take a dictionary of key-value pairs to capture their metadata. This is achieved using the `log_model_metadata` method:
...
ZenML Model and versions are some of the most powerful features in ZenML. To understand them in a deeper way, read the [dedicated Model Management](/usage/development-monitoring/use-the-model-control-plane) guide.
