...
The GCS Artifact Store is an [Artifact Store](/component-gallery/artifact-stores) flavor provided with the GCP ZenML integration that uses [the Google Cloud Storage managed object storage service](https://cloud.google.com/storage/docs/introduction) to store ZenML artifacts in a GCP Cloud Storage bucket.

### When would you want to use it?

Running ZenML pipelines with [the local Artifact Store](/component-gallery/artifact-stores/local) is usually sufficient if you just want to evaluate ZenML or get started quickly without incurring the trouble and the cost of employing cloud storage services in your stack. However, the local Artifact Store becomes insufficient or unsuitable if you have more elaborate needs for your project:
...
Depending on your use-case, however, you may also need to provide additional configuration parameters pertaining to [authentication](/component-gallery/artifact-stores/gcloud-gcs#authentication-methods) to match your deployment scenario.

#### Authentication Methods

Integrating and using a GCS Artifact Store in your pipelines is not possible without employing some form of authentication. ZenML currently provides two options for configuring GCP credentials, the recommended one being to use a [Secrets Manager](/component-gallery/secrets-managers) in your stack to store the sensitive information in a secure location.
...
* [Orchestrators](/component-gallery/orchestrators) need to access the Artifact Store to manage pipeline artifacts

* [Step Operators](/component-gallery/step-operators) need to access the Artifact Store to manage step level artifacts

* [Model Deployers](/component-gallery/model-deployers) need to access the Artifact Store to load served models
...
This method requires using a [Secrets Manager](/component-gallery/secrets-managers) in your stack to store the sensitive GCP authentication information in a secure location.

A Google access key needs to be generated using the [gcloud](https://cloud.google.com/sdk/docs/) utility or Google Cloud console, as covered [here](https://cloud.google.com/docs/authentication/getting-started#creating%5Fa%5Fservice%5Faccount). This comes in the form of a file containing JSON credentials.
...
For more, up-to-date information on the GCS Artifact Store implementation and its configuration, you can have a look at [the API docs](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-gcp/#zenml.integrations.gcp.artifact%5Fstores.gcp%5Fartifact%5Fstore).

### How do you use it?

Aside from the fact that the artifacts are stored in GCP Cloud Storage, using the GCS Artifact Store is no different from [using any other flavor of Artifact Store](/component-gallery/artifact-stores#how-to-use-it).