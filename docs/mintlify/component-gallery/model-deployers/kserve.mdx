...
The KServe Model Deployer is one of the available flavors of the[Model Deployer](/component-gallery/model-deployers) stack component. Provided with the MLflow and Seldon Core integration it can be used to deploy and manage models on an inference server running on top of a Kubernetes cluster.
...
If you are looking for a more easy way to deploy your models locally, you can use the [MLflow Model Deployer](/component-gallery/model-deployers/mlflow) flavor.
...
2. 2.  
KServe needs to be preinstalled and running in the target Kubernetes cluster. Check out the [KServe Serverless installation Guide](https://kserve.github.io/website/0.9/admin/serverless/).

3. 3.  
models deployed with KServe need to be stored in some form of persistent shared storage that is accessible from the Kubernetes cluster where KServe is installed (e.g. AWS S3, GCS, Azure Blob Storage, etc.). You can use one of the supported [remote storage flavors](/component-gallery/artifact-stores) to store your models as part of your stack
...
you can use `kserve_s3` for AWS S3 or `kserve_gs` for GCS and `kserve_az` for Azure. To read more about secrets, secret schemas and how they are used in ZenML, please refer to the [Secrets Manager](/component-gallery/secrets-managers).
...
For more information and a full list of configurable attributes of the KServe secret schemas, check out the [API Docs](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-kserve/#zenml.integrations.kserve.secret%5Fschemas).
...
A concrete example of using the KServe Model Deployer can be found [here](https://github.com/zenml-io/zenml/tree/main/examples/kserve%5Fdeployment).

For more information and a full list of configurable attributes of the KServe Model Deployer, check out the [API Docs](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-kserve/#zenml.integrations.kserve.model%5Fdeployers).

The model deployment step are experimental good for standard use cases. However, if you need to customize the deployment step, you can always create your own model deployment step. Find more information about model deployment steps in the [Model Deployment Steps](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-kserve/#zenml.integrations.kserve.steps) section.
...
The full code example can be found [here](https://github.com/zenml-io/zenml/blob/main/examples/custom%5Fcode%5Fdeployment/).

#### Advanced Custom Code Deployment with KServe Integration

Before creating your custom model class, you should take a look at the ['Deploy Custom Python Model Server with InferenceService'](https://kserve.github.io/website/0.9/modelserving/v1beta1/custom/custom%5Fmodel/) section of the KServe documentation.
...
Example of the [custom model class](https://apidocs.zenml.io/0.13.0/api%5Fdocs/integrations/#zenml.integrations.kserve.custom%5Fdeployer.zenml%5Fcustom%5Fmodel.ZenMLCustomModel)

The built-in KServe custom deployment step responsible for packaging, preparing and deploying to KServe can be found [here](https://apidocs.zenml.io/0.13.0/api%5Fdocs/integrations/#zenml.integrations.kserve.steps.kserve%5Fdeployer.kserve%5Fmodel%5Fdeployer%5Fstep)