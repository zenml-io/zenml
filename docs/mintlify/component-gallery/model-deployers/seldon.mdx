...
The Seldon Core Model Deployer is one of the available flavors of the [Model Deployer](/component-gallery/model-deployers) stack component. Provided with the MLflow integration it can be used to deploy and manage models on an inference server running on top of a Kubernetes cluster.
...
1. 1.  
access to a Kubernetes cluster. The example accepts a `--kubernetes-context` command line argument. This Kubernetes context needs to point to the Kubernetes cluster where Seldon Core model servers will be deployed. If the context is not explicitly supplied to the example, it defaults to using the locally active context.

2. 2.  
Seldon Core needs to be preinstalled and running in the target Kubernetes cluster. Check out the [official Seldon Core installation instructions](https://github.com/SeldonIO/seldon-core/tree/master/examples/auth#demo-setup).
...
you can use `seldon_s3` for AWS S3 or `seldon_gs` for GCS and `seldon_az` for Azure. To read more about secrets, secret schemas and how they are used in ZenML, please refer to the [Secrets Manager](/component-gallery/secrets-managers).
...
* `resources`: the resources to be allocated to the model. This can be configured by passing a dictionary with the `requests` and `limits` keys. The values for these keys can be a dictionary with the `cpu` and `memory` keys. The values for these keys can be a string with the amount of CPU and memory to be allocated to the model.

A concrete example of using the Seldon Core Model Deployer can be found [here](https://github.com/zenml-io/zenml/tree/main/examples/seldon%5Fdeployment).

For more information and a full list of configurable attributes of the Seldon Core Model Deployer, check out the [API Docs](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-seldon/#zenml.integrations.seldon.model%5Fdeployers).
...
Example of the [custom class](https://apidocs.zenml.io/0.13.0/api%5Fdocs/integrations/#zenml.integrations.seldon.custom%5Fdeployer.zenml%5Fcustom%5Fmodel.ZenMLCustomModel).

The built-in Seldon Core custom deployment step responsible for packaging, preparing and deploying to Seldon Core can be found [here](https://apidocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-seldon/#zenml.integrations.seldon.steps.seldon%5Fdeployer.seldon%5Fmodel%5Fdeployer%5Fstep).