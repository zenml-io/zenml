...
- Search on Slack using the built-in Slack search function at the top of the page.  
  ![Searching on Slack.](/images/misc/slack_search_bar.png)
...
- Search the [docs](https://docs.zenml.io) using the search bar in the top right corner of the page.  
  ![Searching on docs page.](/images/misc/doc_search_bar.png)
- Search on https://open.crowd.dev/zenml for issues older than 90 days on our Slack channel.  
  ![Searching on crowd.dev.](/images/misc/opendev_search_bar.png)
- Check out the [Common errors](/misc/debugging#-common-errors) section below.
- Understand the problem by studying the [additional logs](/misc/debugging#-additional-logs) and [client/server logs](/misc/debugging#-view-client-and-server-logs).

Chances are you'd find your answers there. If you can't find any clue, then it's time to post your question on Slack. Join our Slack channel [here](https://zenml.io/slack).
...
See [System Environment Variable](/guidelines/system-environmental-variables) for more information on other environment variables that affect the behavior of ZenML.
...
In the above error snippet, the `step` on line 24 expects an experiment tracker but could not find it on the stack. To solve it, register an experiment tracker of your choice on the stack. For example
...
This also applies to all other [stack components](https://docs.zenml.io/component-gallery/categories). Read more about registering stacks [here](https://docs.zenml.io/starter-guide/stacks/registering-stacks).

### 💢 Install ZenML on Apple Silicon without Rosetta

Our recommended, and tested approach is to install via `Rosetta`.

But a community member recently discovered a way to install ZenML on Apple Silicon without Rosetta. Join the conversation [here](https://open.crowd.dev/zenml/for-what-its-worth-i-was-able-to-successfully-install?q=&p=1).
