...
This connector also supports [automatic configuration and detection of credentials](/usage/resource-data-management/auth-management/service-connectors-guide#auto-configuration) locally configured through the Azure CLI.
...
[Implicit authentication](/usage/resource-data-management/auth-management/best-security-practices#implicit-authentication) to Azure services using environment variables, local configuration files, workload or managed identities.
...
Uses [temporary Azure access tokens](/usage/resource-data-management/auth-management/best-security-practices#short-lived-credentials) explicitly configured by the user or auto-configured from a local environment.
...
┃        │ azure-session-token │ 94d64103-9902-4aa5-8ce4-877061af89af │ 🇦 azure │ 🇦 azure-generic      │     │ ➖     │ default │ 40m58s     │        ┃
...
The Azure Service Connector allows [auto-discovering and fetching credentials](/usage/resource-data-management/auth-management/service-connectors-guide#auto-configuration) and [configuration set up by the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/authenticate-azure-cli) on your local host.
...
2. it doesn't support authenticating to the Azure blob storage service. [The Azure service principal authentication method](/usage/resource-data-management/auth-management/azure-service-connector#azure-service-principal) can be used instead.

</Note>
For an auto-configuration example, please refer to the [section about Azure access tokens](/usage/resource-data-management/auth-management/azure-service-connector#azure-access-token).

## Local client provisioning

The local Azure CLI, Kubernetes `kubectl` CLI and the Docker CLI can be [configured with credentials extracted from or generated by a compatible Azure Service Connector](/usage/resource-data-management/auth-management/service-connectors-guide#configure-local-clients).

<Note>
    Note that the Azure local CLI can only be configured with credentials issued by the Azure Service Connector if the connector is configured with the [service principal authentication method](/usage/resource-data-management/auth-management/azure-service-connector#azure-service-principal).
</Note>
...
┃        │ azure-service-principal │ 3df920bc-120c-488a-b7fc-0e79bc8b021a │ 🇦 azure │ 🇦 azure-generic      │     │ ➖     │ default │            │        ┃
...
The [Azure Artifact Store Stack Component](/stack-components/artifact-stores/azure) can be connected to a remote Azure blob storage container through an Azure Service Connector.
...
## End-to-end examples
<Accordion
    title="AKS Kubernetes Orchestrator, Azure Blob Storage Artifact Store and ACR Container Registry with a multi-type Azure Service Connector"
>
...
* a [Kubernetes Orchestrator](/stack-components/orchestrators/kubernetes) connected to an AKS Kubernetes cluster
* a [Azure Blob Storage Artifact Store](/stack-components/artifact-stores/azure) connected to an Azure blob storage container
* an [Azure Container Registry](/stack-components/container-registries/azure) connected to an ACR container registry
...
1. Register a multi-type Azure Service Connector using the Azure service principal credentials set up at the first step. Note the resources that it has access to:
...
1. register and connect a Kubernetes Orchestrator Stack Component to an AKS cluster:
...
1. Register and connect an Azure Container Registry Stack Component to an ACR container registry:
