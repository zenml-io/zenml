...
```
2. [a ZenML secret](/usage/project-setup/use-secrets/secret-management) can hold the AWS credentials and then be referenced in the S3 Artifact Store configuration attributes:
...
The AWS Service Connector discovered and lifted the AWS Secret Key that was configured on the local machine and securely stored it in the [Secrets Store](/usage/project-setup/use-secrets/secret-management).
