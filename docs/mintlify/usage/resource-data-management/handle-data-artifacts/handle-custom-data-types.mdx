...
A ZenML pipeline is built in a data-centric way. The outputs and inputs of steps define how steps are connected and the order in which they are executed. Each step should be considered as its very own process that reads and writes its inputs and outputs from and to the [artifact store](/versions/0.66.0/stack-components/artifact-stores). This is where **materializers** come into play.
...
In addition to the built-in materializers, ZenML also provides several integration-specific materializers that can be activated by installing the respective [integration](/versions/0.66.0/stack-components/component-guide):
...
<Note>
    If you are running pipelines with a Docker-based [orchestrator](/versions/0.66.0/stack-components/orchestrators), you need to specify the corresponding integration as `required_integrations` in the `DockerSettings` of your pipeline in order to have the integration materializer available inside your Docker container. See the [pipeline configuration documentation](/versions/0.66.0/how-to/use-configuration-files/runtime-configuration) for more information.
</Note>
...
Also, as briefly outlined in the [configuration docs](/versions/0.66.0/how-to/use-configuration-files/what-can-be-configured) section, which materializer to use for the output of what step can also be configured within YAML config files.
...
Check out [this page](/versions/0.66.0/user-guide/starter-guide/manage-artifacts) for information on your step output names and how to customize them.
...
Read more about visualizations [here](/versions/0.66.0/how-to/visualize-artifacts/creating-custom-visualizations).
...
You can learn more about skipping materialization [here](/versions/0.66.0/how-to/handle-data-artifacts/unmaterialized-artifacts).
