...
<Note>
    If you are running pipelines with a Docker-based [orchestrator](/stack-components/orchestrators), you need to specify the corresponding integration as `required_integrations` in the `DockerSettings` of your pipeline in order to have the integration materializer available inside your Docker container. See the [pipeline configuration documentation](/usage/project-setup/use-configuration-files/runtime-configuration) for more information.
</Note>
...
Also, as briefly outlined in the [configuration docs](/usage/project-setup/use-configuration-files/what-can-be-configured) section, which materializer to use for the output of what step can also be configured within YAML config files.
...
Check out [this page](/user-guide/guides/starter-guide/manage-artifacts) for information on your step output names and how to customize them.
...
Read more about visualizations [here](/usage/resource-data-management/visualize-artifacts/creating-custom-visualizations).
...
You can learn more about skipping materialization [here](/usage/resource-data-management/handle-data-artifacts/unmaterialized-artifacts).
