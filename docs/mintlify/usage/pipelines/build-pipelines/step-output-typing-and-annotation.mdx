...
By default, ZenML uses the output name `output` for single output steps and `output_0, output_1, ...` for steps with multiple outputs. These output names are used to display your outputs in the dashboard and [fetch them after your pipeline is finished](/usage/pipelines/build-pipelines/fetching-pipelines).
...
<CardGroup cols={2}>
<Card title="Learn more about output annotation" icon="tag" href="/usage/resource-data-management/handle-data-artifacts/return-multiple-outputs-from-a-step" horizontal />
<Card title="Handle custom data types" icon="cube" href="/usage/resource-data-management/handle-data-artifacts/handle-custom-data-types" horizontal /> </CardGroup>
