...
A step can also be specified as a local user-defined function path (of the form `mymodule.myfile.my_function`). This is particularly useful when defining the hooks via a [YAML Config](/usage/project-setup/use-configuration-files).
...
>
_To set up the local environment used below, follow the recommendations from the_ [_Project templates_](/usage/project-setup/setting-up-a-project-repository/using-project-templates)_._
...
Similar as for regular ZenML steps, you can use the [StepContext](/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-steps) to access information about the current pipeline run or step inside your hook function:
...
Inside the helper function `build_message()`, you will find an example on how developers can work with [StepContext](/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-steps) to form a proper notification:
