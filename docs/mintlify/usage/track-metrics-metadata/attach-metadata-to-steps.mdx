...
Once metadata has been logged in an [artifact](/versions/0.66.0/how-to/track-metrics-metadata/attach-metadata-to-an-artifact), [model](/versions/0.66.0/how-to/track-metrics-metadata/attach-metadata-to-a-model), we can easily fetch the metadata with the ZenML Client:
