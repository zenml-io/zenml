...
* The [resources](/usage/resource-data-management/training-with-gpus#specify-resource-requirements-for-steps) required for a step
* Configuring the [containerization](/usage/project-setup/customize-docker-builds) process of a pipeline (e.g. What requirements get installed in the Docker image)
...
* **General settings** that can be used on all ZenML pipelines. Examples of these are:
   * [DockerSettings](/usage/project-setup/customize-docker-builds) to specify Docker settings.
   * [ResourceSettings](/usage/resource-data-management/training-with-gpus) to specify resource settings.
