...
For brevity, we have removed all possible keys. To view a sample file with all possible keys, refer to [this page](/usage/project-setup/use-configuration-files/autogenerate-a-template-yaml-file).
...
* `enable_artifact_metadata`: Whether to [associate metadata with artifacts or not](/usage/resource-data-management/handle-data-artifacts/handle-custom-data-types#optional-which-metadata-to-extract-for-the-artifact).
* `enable_artifact_visualization`: Whether to [attach visualizations of artifacts](/usage/resource-data-management/visualize-artifacts).
* `enable_cache`: Utilize [caching](/usage/pipelines/build-pipelines/control-caching-behavior) or not.
* `enable_step_logs`: Enable tracking [step logs](/usage/development-monitoring/control-logging/enable-or-disable-logs-storing).
...
The UUID of the [build](/usage/project-setup/customize-docker-builds) to use for this pipeline. If specified, Docker image building is skipped for remote orchestrators, and the Docker image specified in this build is used.
...
Specifies the ZenML [Model](/user-guide/guides/starter-guide/track-ml-models) to use for this pipeline.
...
A dictionary of JSON-serializable [parameters](/usage/pipelines/build-pipelines/use-pipeline-step-parameters) specified at the pipeline or step level. For example:
...
Settings are special runtime configurations of a pipeline or a step that require a [dedicated section](/usage/project-setup/use-configuration-files/runtime-configuration). In short, they define a bunch of execution configuration such as Docker building and resource settings.
...
<Info>
  Find a complete list of all Docker Settings [here](https://sdkdocs.zenml.io/latest/core%5Fcode%5Fdocs/core-config/#zenml.config.docker%5Fsettings.DockerSettings). To learn more about pipeline containerization consult our documentation on this [here](/usage/project-setup/customize-docker-builds).
</Info>
...
The `source` of the [failure and success hooks](/usage/pipelines/build-pipelines/use-failure-success-hooks) can be specified.
...
* `outputs`: This is configuration of the output artifacts of this step. This is further keyed by output name (by default, step outputs [are named output](/usage/resource-data-management/handle-data-artifacts/return-multiple-outputs-from-a-step)). The most interesting configuration here is the `materializer_source`, which is the UDF path of the materializer in code to use for this output (e.g. `materializers.some_data.materializer.materializer_class`). Read more about this source path [here](/usage/resource-data-management/handle-data-artifacts/handle-custom-data-types).
