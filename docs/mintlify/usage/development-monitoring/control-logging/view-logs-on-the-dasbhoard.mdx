...
* In case of a deployed ZenML server, logs for runs on a [local artifact store](/stack-components/artifact-stores/local) will not be accessible. Logs for runs using a [remote artifact store](/user-guide/guides/production-guide/remote-storage) **may be** accessible, if the artifact store has been configured with a [service connector](/usage/resource-data-management/auth-management/service-connectors-guide). Please read [this chapter](/user-guide/guides/production-guide/remote-storage) of the production guide to learn how to configure a remote artifact store with a service connector.
...
<Note>
    If you do not want to store the logs for your pipeline (for example due to performance reduction or storage limits), you can follow [these instructions](/usage/development-monitoring/control-logging/enable-or-disable-logs-storing).
</Note>
