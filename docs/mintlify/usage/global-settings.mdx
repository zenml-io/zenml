...
* `zenml downgrade` \- downgrade the ZenML version in the global configuration to match the version of the ZenML package installed in the current environment. Read more about this in the [ZenML Version Mismatch](versions/0.66.0/reference/global-settings#version-mismatch-downgrading) section.
</Note>
