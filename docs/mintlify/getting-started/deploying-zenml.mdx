...
**Tip**: In order to skip deploying ZenML completely, or if you just want to get up and running quickly, you can use an early version of the [ZenML Cloud](https://zenml.io/cloud) where we'll give you a free and managed ZenML server!
...
ZenML relies on a SQLAlchemy-compatible database to store all its data: stack configurations, pipeline runs etc. The location and type of this database can be freely chosen by the user. By default, a SQLite database is stored on your local host (see [Scenario 1](/getting-started/deploying-zenml#running-zenml-locally)).
...
This is likely the first experience that users will run into. Simply `pip install zenml` and dive right in without having to worry about a thing. Your stacks, stack components and pipeline runs all are stored in a SQLite database located in your global configuration directory (e.g. at `~/.config/zenml/local_stores/default_zen_store/zenml.db` if you're running on Linux). The ZenML client creates and directly connects to this database. You don't need to worry about a thing.

![Scenario 1](/images/getting-started/Scenario1.png)

ZenML on SQLite {/* TODO caption */}

Users that wish to be fancy about how they connect to the database can find out how to configure a different location for the SQLite database, or even a different database altogether [here](/guidelines/global-config).
...
> If you are looking for a quick deployment without having to worry about configuring the right access, the [zenml deploy CLI command ](/getting-started/deploying-zenml/cli)is the way to go!

**Tip**: In order to quickly get access to a ZenML server deployed in the cloud without the hassle of setting it up and maintaining it, you can get a free, early version of the [ZenML Cloud](https://zenml.io/cloud) by filling out this form instead!
...
![Scenario 3](/images/getting-started/Scenario3.2.png)
...
- The [zenml deploy CLI command](/getting-started/deploying-zenml/cli) that provides an easy interface to deploy on Kubernetes in AWS, GCP or Azure.

- A [Docker image](/getting-started/deploying-zenml/docker) that you can run in any environment of your choice.

- A [Helm chart](/getting-started/deploying-zenml/helm) that can be deployed to any Kubernetes cluster (on-prem or managed).
