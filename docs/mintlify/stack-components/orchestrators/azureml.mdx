...
In order to use an AzureML orchestrator, you need to first deploy [ZenML to the cloud](/getting-started/deploying-zenml). It would be recommended to deploy ZenML in the same region as you plan on using for AzureML, but it is not necessary to do so. You must ensure that you are [connected to the remote ZenML server](/usage/resource-data-management/connecting-to-zenml/connect-in-with-your-user-interactive) before using this stack component.
