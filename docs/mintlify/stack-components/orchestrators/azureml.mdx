...
<Info>
    Would you like to skip ahead and deploy a full ZenML cloud stack already, including an AzureML orchestrator? Check out the [in-browser stack deployment wizard](/stack-components/stack-deployment/deploy-a-cloud-stack), the [stack registration wizard](/stack-components/stack-deployment/register-a-cloud-stack), or [the ZenML Azure Terraform module](/stack-components/stack-deployment/deploy-a-cloud-stack-with-terraform) for a shortcut on how to deploy & register this stack component.
</Info>

In order to use an AzureML orchestrator, you need to first deploy [ZenML to the cloud](/versions/0.66.0/getting-started/deploying-zenml). It would be recommended to deploy ZenML in the same region as you plan on using for AzureML, but it is not necessary to do so. You must ensure that you are [connected to the remote ZenML server](/versions/0.66.0/how-to/connecting-to-zenml/connect-in-with-your-user-interactive) before using this stack component.
...
For each pipeline run, ZenML will build a Docker image called `<CONTAINER_REGISTRY_URI>/zenml:<PIPELINE_NAME>` which includes your code and use it to run your pipeline steps in AzureML. Check out [this page](/usage/project-setup/customize-docker-builds) if you want to learn more about how ZenML builds these images and how you can customize them.
...
<Frame caption="AzureML pipeline example">
  <img src="/_assets/stack-components/stack-components-7.avif"/>
</Frame>
