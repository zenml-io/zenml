...
* the Data Validator is an optional type of Stack Component that needs to be registered as part of your ZenML [Stack](/user-guide/guides/production-guide/understand-stacks).
* Data Validators used in ZenML pipelines usually generate data profiles and data quality check reports that are versioned and stored in the [Artifact Store](/stack-components/artifact-stores) and can be [retrieved and visualized](/usage/resource-data-management/visualize-artifacts) later.
...
* you can access the data validation artifacts in subsequent pipeline steps, or [fetch them afterwards](/usage/resource-data-management/handle-data-artifacts/load-artifacts-into-memory) to process them or visualize them as needed.
