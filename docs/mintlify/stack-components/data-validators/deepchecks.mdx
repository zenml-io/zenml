...
Then, place the following snippet above your pipeline definition. Note that the path of the `dockerfile` are relative to where the pipeline definition file is. Read [the containerization guide](/versions/0.66.0/how-to/customize-docker-builds) for more details:
...
Alternatively, if you are running inside a Jupyter notebook, you can load and render the suites and results using the [artifact.visualize() method](/versions/0.66.0/how-to/visualize-artifacts), e.g.:
