...
<Info>
Instead of hardcoding a step operator name, you can also use the [Client](sdk/python-client) to dynamically use the step operator of your active stack:
...
For additional configuration of the Spark step operator, you can pass `SparkStepOperatorSettings` when defining or running your pipeline. Check out the [SDK docs](https://sdkdocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-spark/#zenml.integrations.spark.flavors.spark%5Fstep%5Foperator%5Fflavor.SparkStepOperatorSettings) for a full list of available attributes and [this docs page](/usage/project-setup/use-configuration-files/runtime-configuration) for more information on how to specify settings.
