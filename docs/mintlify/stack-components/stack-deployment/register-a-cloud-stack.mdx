...
To register the cloud stack, the first thing that the wizard needs is a [service connector](/usage/resource-data-management/auth-management/service-connectors-guide). You can either use an existing connector by providing its ID or name `-sc <SERVICE_CONNECTOR_ID_OR_NAME>` (CLI-Only) or the wizard will create one for you.
