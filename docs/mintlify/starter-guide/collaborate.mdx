...
As mentioned before, the most common starting point for most ZenML users would be to locally install it and start managing stacks. We have discussed the nitty-gritty details of such usage [previously](/starter-guide/stacks/managing-stacks).

![](/images/starter-guide/01_local_stack.png)

Working with local ZenML

## Step 2: Single user working with local and cloud stacks

The next step in the journey is to go to the cloud, and for that you need to [deploy ZenML](/getting-started/deploying-zenml). Don't worry - we will show an easy way of getting started with this quickly, with little to no knowledge of cloud technologies in the [next section](/starter-guide/collaborate/zenml-deployment).

For now, we should understand the consequence of doing such an action. Once ZenML is deployed and the user connects to it remotely, all stacks, pipelines, runs, and other metadata will be [centrally tracked](/advanced-guide/pipelines/settings) in the database that backs the server.

The user can still keep using local, [non-shared](/starter-guide/stacks/managing-stacks#sharing-stacks-over-a-zenml-server) stacks (e.g. the default stack). However, they will notice a significant dropoff in speed of execution of the pipelines because they are now communicating over the internet to a central location.

![](/images/starter-guide/02_multiple_stacks.png)

Single user working with local and cloud stacks
...
* [Kubeflow**Orchestrator**](/component-gallery/orchestrators/kubeflow) which orchestrates your ML workflows on [Kubeflow Pipelines](https://www.kubeflow.org/docs/components/pipelines/v1/introduction/).

* [S3**ArtifactStore**](/component-gallery/artifact-stores/amazon-s3) which can store your artifacts in a [S3 storage](https://aws.amazon.com/s3/).

* [MLflow**ExperimentTracker**](/component-gallery/experiment-trackers/mlflow) which can track your experiments with [MLFlow](https://mlflow.org/).

* [Evidently**DataValidator**](/component-gallery/data-validators/evidently) which can help you validate your data with [Evidently](https://www.evidentlyai.com/).

Once a stack such as the above is [registered](/starter-guide/stacks/registering-stacks) and [set active](/starter-guide/stacks/managing-stacks#setting-the-local-active-stack), then running a ZenML pipeline will result in a much more different behavior then before.
...
The moment the stack is shared, other users who [connect to the server](/starter-guide/collaborate/zenml-deployment) will be able to see the stack and use it as well!

![](/images/starter-guide/03_multiple_users.png)

Multiple users working with local and cloud stacks

So, as you can see it all starts with a [deploying ZenML](/getting-started/deploying-zenml)! So how do we do that - let's go into the [next section](/starter-guide/collaborate/zenml-deployment) to learn how to do it in a beginner-friendly way.