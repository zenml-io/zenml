...
If you are new to MLOps and would like to learn more about ML pipelines in general, checkout [ZenBytes](https://github.com/zenml-io/zenbytes), our lesson series on practical MLOps, where we introduce ML pipelines in more detail in [ZenBytes lesson 1.1](https://github.com/zenml-io/zenbytes/blob/main/1-1%5FPipelines.ipynb).
...
As this step has multiple outputs, we need to use the `zenml.steps.step_output.Output` class to indicate the names of each output. These names can be used to directly access the outputs of steps after running a pipeline, as we will see [in a later chapter](/starter-guide/pipelines/fetching-pipelines).
...
We will dive deeper into how to inspect the finished run within the chapter on [Accessing Pipeline Runs](/starter-guide/pipelines/fetching-pipelines).

### Inspect your pipeline in the dashboard

Notice the last log, that indicates running a command to view the dashboard. Check out the dashboard guide [in the next section](/starter-guide/pipelines/dashboard) to inspect your pipeline there.
...
Once a pipeline has been executed, it is represented by a [PipelineSpec](https://apidocs.zenml.io) that uniquely identifies it. Therefore, you cannot edit a pipeline after it has been run once. In order to iterate quickly pipelines, there are three options:
...
* Pipelines can be given [unique names](/starter-guide/pipelines#give-each-pipeline-run-a-name) each time they are run to uniquely identify them.

We will dive into quickly iterating over pipelines [later in this section](https://github.com/zenml-io/zenml/blob/release/0.31.1/docs/book/starter-guide/pipelines/iterating.md).
