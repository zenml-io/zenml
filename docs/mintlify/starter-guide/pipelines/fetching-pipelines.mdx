...
The `pipeline_configuration` is a super object that contains all configuration of the pipeline and pipeline run, including [pipeline-level BaseSettings](/advanced-guide/pipelines/settings), which we will learn more about later. You can also access the settings directly via the `settings` variable.
...
The steps are ordered by time of execution. Depending on the [orchestrator](/component-gallery/orchestrators), steps can be run in parallel. Thus, accessing steps by index can be unreliable across different runs, and it is recommended to access steps by the step class, an instance of the class or even the name of the step as a string: `get_step(step=...)` instead.

Similar to the run, for reproducibility, you can use the `step` object to access:

* [BaseParameters](https://github.com/zenml-io/zenml/blob/release/0.31.1/docs/book/starter-guide/pipelines/iterating.md) via `step.parameters`: The parameters used to run the step.

* [Step-level BaseSettings](/advanced-guide/pipelines/settings) via `step.step_configuration`
...
You can get a lot more metadata within a step as well, something we'll learn in more detail in the [advanced docs](/advanced-guide/pipelines/step-metadata).