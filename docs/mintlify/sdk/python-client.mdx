...
* **Schedules**: Metadata about the schedules that you have used to [schedule pipeline runs](/usage/pipelines/build-pipelines/schedule-a-pipeline).
* **Builds**: The pipeline-specific Docker images that were created when [containerizing your pipeline](/usage/project-setup/customize-docker-builds).
...
<Info>
Checkout the [documentation on fetching runs](/usage/pipelines/build-pipelines/fetching-pipelines) for more information on the various ways how you can fetch and use the pipeline, pipeline run, step run, and artifact resources in code.
</Info>
...
* **Flavors**: The [stack component flavors](/getting-started/core-concepts#flavor) available to you, including:
   ...
   * Custom flavors that you have [created yourself](/stack-components/stack-deployment/implement-a-custom-stack-component).
* **User**: The users registered in your ZenML instance. If you are running locally, there will only be a single `default` user.
* **Secrets**: The infrastructure authentication secrets that you have registered in the [ZenML Secret Store](/usage/project-setup/use-secrets/interact-with-secrets).
* **Service Connectors**: The service connectors that you have set up to [connect ZenML to your infrastructure](/usage/resource-data-management/auth-management).
