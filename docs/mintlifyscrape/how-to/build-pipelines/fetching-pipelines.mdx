...
Check out the [ZenML Client Documentation](/reference/python-client) for more information on the `Client` class and its purpose.
...
Alternatively, you can also use the `pipeline_model.get_runs()` method which allows you to specify detailed parameters for filtering or pagination. See the [ZenML SDK Docs](/reference/python-client#list-of-resources) for more information.
...
The `pipeline_configuration` is an object that contains all configurations of the pipeline and pipeline run, including the [pipeline-level settings](/user-guide/production-guide/configure-pipeline), which we will learn more about later:
...
If you're only calling each step once inside your pipeline, the **invocation ID** will be the same as the name of your step. For more complex pipelines, check out [this page](/how-to/build-pipelines/using-a-custom-step-invocation-id) to learn more about the invocation ID.

### Inspect pipeline runs with our VS Code extension

![](/images/how-to/build-pipelines/files%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FSXceRPgF1t4PqY7HbePb%252Fuploads%252Fgit-blob-c37db3c6e830815eec7bed02bb5207c816a24e95%252Fzenml-extension-shortened.gif)
...
Check out [this page](/user-guide/starter-guide/manage-artifacts#giving-names-to-your-artifacts) to see what the output names of your steps are and how to customize them.
...
We will talk more about metadata [in the next section](/user-guide/starter-guide/manage-artifacts#logging-metadata-for-an-artifact).
...
As shown in the example, we can get additional information about the current run using the `StepContext`, which is explained in more detail in the [advanced docs](/how-to/track-metrics-metadata/fetch-metadata-within-steps).
