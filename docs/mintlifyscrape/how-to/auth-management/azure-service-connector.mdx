...
This connector also supports [automatic configuration and detection of credentials](/how-to/auth-management/service-connectors-guide#auto-configuration) locally configured through the Azure CLI.
...
[Implicit authentication](/how-to/auth-management/best-security-practices#implicit-authentication) to Azure services using environment variables, local configuration files, workload or managed identities.
...
Uses [temporary Azure access tokens](/how-to/auth-management/best-security-practices#short-lived-credentials) explicitly configured by the user or auto-configured from a local environment.
...
The Azure Service Connector allows [auto-discovering and fetching credentials](/how-to/auth-management/service-connectors-guide#auto-configuration) and [configuration set up by the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/authenticate-azure-cli) on your local host.
...
2. it doesn't support authenticating to the Azure blob storage service. [The Azure service principal authentication method](/how-to/auth-management/azure-service-connector#azure-service-principal) can be used instead.

For an auto-configuration example, please refer to the [section about Azure access tokens](/how-to/auth-management/azure-service-connector#azure-access-token).

## Local client provisioning

The local Azure CLI, Kubernetes `kubectl` CLI and the Docker CLI can be [configured with credentials extracted from or generated by a compatible Azure Service Connector](/how-to/auth-management/service-connectors-guide#configure-local-clients).

Note that the Azure local CLI can only be configured with credentials issued by the Azure Service Connector if the connector is configured with the [service principal authentication method](/how-to/auth-management/azure-service-connector#azure-service-principal).
...
1. register and connect an Azure Blob Storage Artifact Store Stack Component to an Azure blob container:  
Copy  
```  
