...
As each component defines a different interface, make sure to check out the base class definition of the component type that you want to implement and also check out the [documentation on how to extend specific stack components](/how-to/stack-deployment/implement-a-custom-stack-component#extending-specific-stack-components).

If you would like to automatically track some metadata about your custom stack component with each pipeline run, you can do so by defining some additional methods in your stack component implementation class as shown in the [Tracking Custom Stack Component Metadata](/how-to/track-metrics-metadata/fetch-metadata-within-steps) section.
...
You can read more about the differences [here](/how-to/use-configuration-files/runtime-configuration).
...
You can pass sensitive configuration values as [secrets](/how-to/interact-with-secrets) by defining them as type `SecretField` in the configuration class.
