...
* Using a YAML configuration file as described [here](/how-to/use-configuration-files):
...
Check out [this page](/how-to/use-configuration-files/configuration-hierarchy) for more information on the hierarchy and precedence of the various ways in which you can supply the settings.

### Specifying Docker build options

If you want to specify build options that get passed to the build method of the [image builder](/how-to/configure-python-environments#image-builder-environment). For the default local image builder, these options get passed to the [docker build command](https://docker-py.readthedocs.io/en/stable/images.html#docker.models.images.ImageCollection.build).
