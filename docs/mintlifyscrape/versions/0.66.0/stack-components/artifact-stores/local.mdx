...
The local Artifact Store is a built-in ZenML [Artifact Store](/versions/0.66.0/stack-components/artifact-stores) flavor that uses a folder on your local filesystem to store artifacts.
...
<Note>
  The local Artifact Store is not meant to be utilized in production. The local filesystem cannot be shared across your team and the artifacts stored in it cannot be accessed from other machines. This also means that [artifact visualizations](/versions/0.66.0/how-to/visualize-artifacts) will not be available when using a local Artifact Store through a [ZenML instance deployed in the cloud](/versions/0.66.0/getting-started/deploying-zenml).
...
* only [Orchestrators](/versions/0.66.0/stack-components/orchestrators) running on the local machine, such as the [local Orchestrator](/versions/0.66.0/stack-components/orchestrators/local), a [local Kubeflow Orchestrator](/versions/0.66.0/stack-components/orchestrators/kubeflow), or a [local Kubernetes Orchestrator](/versions/0.66.0/stack-components/orchestrators/kubernetes) can be combined with a local Artifact Store
* only [Model Deployers](/versions/0.66.0/stack-components/model-deployers) that are running locally, such as the [MLflow Model Deployer](/versions/0.66.0/stack-components/model-deployers/mlflow), can be used in combination with a local Artifact Store
* [Step Operators](/versions/0.66.0/stack-components/step-operators): none of the Step Operators can be used in the same stack as a local Artifact Store, given that their very purpose is to run ZenML steps in remote specialized environments
...
Aside from the fact that the artifacts are stored locally, using the local Artifact Store is no different from [using any other flavor of Artifact Store](/versions/0.66.0/stack-components/artifact-stores#how-to-use-it).
