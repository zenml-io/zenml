...
* you have access to SageMaker. If you're using a different cloud provider, take a look at the [Vertex](/versions/0.66.0/stack-components/step-operators/vertex) or [AzureML](/versions/0.66.0/stack-components/step-operators/azureml) step operators.
...
* An IAM role with the correct permissions. See the [deployment section](/versions/0.66.0/stack-components/step-operators/sagemaker#how-to-deploy-it) for detailed instructions.
* An [AWS container registry](/versions/0.66.0/stack-components/container-registries/aws) as part of our stack. Take a look [here](/versions/0.66.0/stack-components/container-registries/aws#how-to-deploy-it) for a guide on how to set that up.
* A [remote artifact store](/versions/0.66.0/stack-components/artifact-stores) as part of your stack. This is needed so that both your orchestration environment and SageMaker can read and write step artifacts. Check out the documentation page of the artifact store you want to use for more information on how to set that up and configure authentication for it.
...
* If using a [local orchestrator](/versions/0.66.0/stack-components/orchestrators/local): ZenML will try to implicitly authenticate to AWS via the `default` profile in your local [AWS configuration file](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html). Make sure this profile has permissions to create and manage SageMaker runs (e.g. [the AmazonSageMakerFullAccess managed policy](https://docs.aws.amazon.com/sagemaker/latest/dg/security-iam-awsmanpol.html) permissions).
...
<Info>
    ZenML will build a Docker image called `<CONTAINER_REGISTRY_URI>/zenml:<PIPELINE_NAME>` which includes your code and use it to run your steps in SageMaker. Check out [this page](/versions/0.66.0/how-to/customize-docker-builds) if you want to learn more about how ZenML builds these images and how you can customize them.
</Info>

#### Additional configuration

For additional configuration of the SageMaker step operator, you can pass `SagemakerStepOperatorSettings` when defining or running your pipeline. Check out the [SDK docs](https://sdkdocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-aws/#zenml.integrations.aws.flavors.sagemaker%5Fstep%5Foperator%5Fflavor.SagemakerStepOperatorSettings) for a full list of available attributes and [this docs page](/versions/0.66.0/how-to/use-configuration-files/runtime-configuration) for more information on how to specify settings.
