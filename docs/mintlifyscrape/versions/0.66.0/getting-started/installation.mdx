...
ZenML comes bundled with a web dashboard that lives inside a [sister repository](https://github.com/zenml-io/zenml-dashboard). In order to get access to the dashboard **locally**, you need to launch the [ZenML Server and Dashboard locally](/versions/0.66.0/getting-started/deploying-zenml). For this, you need to install the optional dependencies for the ZenML Server:
...
However, advanced ZenML features are dependent on a centrally-deployed ZenML server accessible to other MLOps stack components. You can read more about it [here](/versions/0.66.0/getting-started/deploying-zenml).

For the deployment of ZenML, you have the option to either [self-host](/versions/0.66.0/getting-started/deploying-zenml) it or register for a free [ZenML Pro](https://cloud.zenml.io/signup?utm%5Fsource=docs&utm%5Fmedium=referral%5Flink&utm%5Fcampaign=cloud%5Fpromotion&utm%5Fcontent=signup%5Flink) account.
