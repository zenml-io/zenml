...
* [Retrieval evaluation](/user-guide/llmops-guide/evaluation/retrieval), so checking that the retrieved documents or document chunks are relevant to the query.
* [Generation evaluation](/user-guide/llmops-guide/evaluation/generation), so checking that the generated text is coherent and helpful for our specific use case.

![](/images/user-guide/llmops-guide/files%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FSXceRPgF1t4PqY7HbePb%252Fuploads%252Fgit-blob-f00b90cd17a312af12fbeb53ff02618f02e3de04%252Fevaluation-two-parts.png)
...
These are just examples, and the specific metrics and methods you use will depend on your use case. The [generation evaluation](/user-guide/llmops-guide/evaluation/generation) functions as an end-to-end evaluation of the RAG pipeline, as it checks the final output of the system. It's during these end-to-end evaluations that you'll have most leeway to use subjective metrics, as you're evaluating the system as a whole.

Before we dive into the details, let's take a moment to look at [a short high-level code example](/user-guide/llmops-guide/evaluation/evaluation-in-65-loc) showcasing the two main areas of evaluation. Afterwards the following sections will cover the two main areas of evaluation in more detail [as well as offer practical guidance](/user-guide/llmops-guide/evaluation/evaluation-in-practice) on when to run these evaluations and what to look for in the results.
