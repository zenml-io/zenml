...
These, along with [remote storage](/user-guide/production-guide/remote-storage), complete a basic cloud stack where our pipeline is entirely running on the cloud.
...
![](/images/user-guide/production-guide/files%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FSXceRPgF1t4PqY7HbePb%252Fuploads%252Fgit-blob-e6f565d0a09e2bc195318d690876ae6e480fefd2%252Fcloud_orchestration_run.png)
...
Before we start registering any components, there is another step that we have to execute. As we [explained in the previous section](/user-guide/production-guide/remote-storage#configuring-permissions-with-your-first-service-connector), components such as orchestrators and container registries often require you to set up the right permissions. In ZenML, this process is simplified with the use of [Service Connectors](/how-to/auth-management). For this example, we need to use the [IAM role authentication method of our AWS service connector](/how-to/auth-management/aws-service-connector#aws-iam-role):
...
Now that we have our orchestrator and container registry registered, we can [register a new stack](/user-guide/production-guide/understand-stacks#registering-a-stack), just like we did in the previous chapter:
...
Now, using the [code from the previous chapter](/user-guide/production-guide/understand-stacks#run-a-pipeline-on-the-new-local-stack), we can run a training pipeline. First, set the minimal cloud stack active:
