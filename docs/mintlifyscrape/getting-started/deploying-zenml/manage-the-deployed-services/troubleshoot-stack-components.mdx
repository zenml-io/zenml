...
1. Get the name of the deployed cluster.`zenml stack recipe output eks-cluster-name`
...
3. Run the following command.`aws eks update-kubeconfig --name <NAME> --region <REGION>`

1. Get the name of the deployed cluster.`zenml stack recipe output gke-cluster-name`\\
...
4. Run the following command.`gcloud container clusters get-credentials <NAME> --region <REGION> --project <PROJECT_ID>`
...
1. Get the name of the deployed cluster.`zenml stack recipe output k3d-cluster-name`\\
2. Set the `KUBECONFIG` env variable to the `kubeconfig` file from the cluster.`export KUBECONFIG=$(k3d kubeconfig get <NAME>)`\\
