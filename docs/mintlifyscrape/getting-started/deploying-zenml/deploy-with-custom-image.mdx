...
* You have implemented a custom artifact store for which you want to enable [artifact visualizations](https://github.com/zenml-io/zenml/blob/release/0.66.0/docs/book/how-to/handle-data-artifacts/visualize-artifacts.md) or [step logs](/how-to/setting-up-a-project-repository/best-practices#logging) in your dashboard.
...
Deploying ZenML with custom Docker images is only possible for [Docker](/getting-started/deploying-zenml/deploy-with-docker) or [Helm](/getting-started/deploying-zenml/deploy-with-helm) deployments.
...
If you want to verify your custom image locally, you can follow the [Deploy a custom ZenML image via Docker](/getting-started/deploying-zenml/deploy-with-custom-image#deploy-a-custom-zenml-image-via-docker) section below to deploy the ZenML server locally first.
...
1. Define a custom `config.yaml` based on the [base deployment configuration file](/getting-started/deploying-zenml/deploy-with-zenml-cli#base-configuration-file) and set `zenmlserver_image_repo` and `zenmlserver_image_tag` according to the custom image you built:  
Copy  
```  
...
See the general [ZenML CLI Deployment Guide](/getting-started/deploying-zenml/deploy-with-zenml-cli) for more information on how to use the `zenml deploy` CLI command and what other options can be configured.

#### Deploy a custom ZenML image via Docker

To deploy your custom image via Docker, first familiarize yourself with the general [ZenML Docker Deployment Guide](/getting-started/deploying-zenml/deploy-with-docker).
...
To deploy your custom image via Helm, first familiarize yourself with the general [ZenML Helm Deployment Guide](/getting-started/deploying-zenml/deploy-with-helm).
