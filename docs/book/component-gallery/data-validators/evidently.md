...
to display and interact with the Evidently reports generated by your pipelines. 

#### The Evidently Report step

ZenML wraps the Evidently functionality in the form of a standard
Evidently report pipeline step. You select which metrics you want to generate in
your step by passing a list of metric configurations in your
`EvidentlyReportParameters` step configuration:
...
- [Drift Detection with Evidently](https://github.com/zenml-io/zenml/tree/main/examples/evidently_data_validation)


### Data Validation

You can also use the Evidently Data Validator to validate your data using Evidently's Tests. 

You can generate results in the form of a `TestSuite` object that can be viewed as a JSON and an HTML string.

Similar to the profiling actions using `Report`, you can instantiate, configure and insert [the standard `EvidentlyTestStep`](#the-evidently-test-step)
shipped with ZenML into your pipelines. This is the easiest way and the
recommended approach, but can only be customized through the supported step
configuration parameters.
...
ZenML wraps the Evidently functionality in the form of a standard
`EvidentlyTestStep` step. You select which metrics you want to generate in
...
All tests and test presets that can be used in the `tests` parameter are available on the Evidently [documentation page](https://docs.evidentlyai.com/reference/all-tests).

The tests parameter can be passed a list of tests, test presets or a dictionary of
tests to use with the generate_column_tests method.
- For tests and test presets that are on the dataset level or those
that don't require any parameters, you can use a string with the exact name as in the evidently library.
- For tests and test presets that are on the column level or those
that require parameters like "column_name", you can use a list with the exact name of the test as in the evidently library and a dictionary with the parameters.
- Pass a dictionary when you want to choose a test for more than
one columns.This test should also necessarily have the "column_name" parameter. The structure of the dictionary should be as follows:
{
    "test": "test_name",
    "parameters": {},
    "columns": ["column1", "column2"]
...
As can be seen from the [step definition](https://apidocs.zenml.io/latest/integration_code_docs/integrations-evidently/#zenml.integrations.evidently.steps.evidently_test.EvidentlyTestStep), the step takes in
a reference dataset and a comparison dataset required for data drift and
model comparison reports. It returns the Evidently `TestSuite` object in the JSON and HTML formats:
