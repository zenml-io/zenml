...
* **ZENML\_SERVER\_OTEL\_EXPORTER\_OTLP\_ENDPOINT**: Set this to the base OTLP/HTTP endpoint of an OpenTelemetry-compatible backend to export ZenML server traces, metrics, and logs. ZenML appends `/v1/traces`, `/v1/metrics`, and `/v1/logs` for each signal. If this variable is not set, server OpenTelemetry instrumentation is disabled. The server logs export to the OTel backend is independent of `ZENML_CONSOLE_LOGGING_FORMAT`: logs are exported as OTLP records with structured attributes derived from the underlying log record, not as console-formatted text or JSON.
