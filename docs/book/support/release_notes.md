...
0.5.0 is a complete API change from the previous versions of ZenML, and is a _breaking_ upgrade. Fundamental
...
cd zenml_enabled_repo rm -rf pipelines/ rm -rf .zenml/

```text
And then another ZenML init:
```

pip install --upgrade zenml cd zenml_enabled_repo zenml init
