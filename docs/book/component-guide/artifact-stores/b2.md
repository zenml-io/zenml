...
3. **Export the credentials as environment variables.** ZenML's B2 flavor reads Backblaze's standard environment variable names if no key/secret is configured directly on the artifact store:
...
If `B2_APPLICATION_KEY_ID` and `B2_APPLICATION_KEY` are exported in the shell that runs ZenML, no further authentication configuration is required — the flavor will pick them up automatically. To keep credentials out of the shell environment, store them in a [ZenML Secret](https://docs.zenml.io/getting-started/deploying-zenml/secret-management) and reference them on registration:
...
By default, the B2 flavor targets the `us-west-004` region by injecting `https://s3.us-west-004.backblazeb2.com` as the `client_kwargs.endpoint_url` whenever the user has not configured one. If your bucket lives in a different B2 region, override the endpoint at registration time:
...
Use the S3 endpoint shown for your bucket in the Backblaze console (it always has the form `https://s3.<region>.backblazeb2.com`).
...
For more, up-to-date information on the B2 Artifact Store implementation and its configuration, see the integration source at `src/zenml/integrations/b2/` in the ZenML repository. An auto-generated SDK docs page at `https://sdkdocs.zenml.io/latest/integration_code_docs/integrations-b2.html` will be available once this integration ships in a published release.
