...
- `monitoring_interval`: Poll interval (seconds) for workload status.

**Per-deployment settings (`RunAIDeployerSettings`):**

- Compute: `gpu_devices_request`, `gpu_request_type` (`portion` or `memory`), `gpu_portion_request`, `gpu_memory_request`, `cpu_core_request`, `cpu_memory_request`
- Scaling: `min_replicas`, `max_replicas`
- Scheduling: `node_pools`, `node_type`
- Metadata & runtime: `labels`, `annotations`, `environment_variables`, `workload_name_prefix`
...
- **503 “too many 503 error responses”**: temporary Run:AI control-plane unavailability or networking issues. Retry; if persistent, check your org’s Run:AI status and outbound network/proxy settings.
