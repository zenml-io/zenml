...
{% hint style="info" %}
Stacks can have multiple step operators attached. If you use `@step(step_operator=True)`, ZenML uses the default attached step operator, which is the first step operator of that type in the stack. If you want to use a non-default step operator, specify it explicitly by name in the `@step` decorator.
{% endhint %}
