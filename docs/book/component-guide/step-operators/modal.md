...
The step operator runs every step in a Modal Sandbox inside one Modal App, named `zenml-step-operator` by default. You can change it with the `app_name` option on the step operator. Each sandbox is tagged with `zenml-step-run-id` and `zenml-step`. While a step is running, you can find its sandbox with `modal.Sandbox.list(tags={"zenml-step-run-id": "<STEP_RUN_ID>"})`; after it finishes, use the `sandbox_id` stored in the step run metadata.
