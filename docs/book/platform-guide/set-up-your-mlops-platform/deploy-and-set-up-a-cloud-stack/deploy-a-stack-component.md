...
Using the values you pass for the cloud, the CLI picks up the right modular recipee to use (one of AWS, GCP, or k3d) and then deploys that recipe with the speciifc stack component enabled.&#x20;
...
This variable is then passed as input to the underlying modular recipe. If you check the [`variables.tf`](https://github.com/zenml-io/mlops-stacks/blob/main/gcp-modular/variables.tf) file for a given recipe, you can find all of the supported flavor-stack component combinations there.
