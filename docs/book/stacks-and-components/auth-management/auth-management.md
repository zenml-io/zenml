...
1.  the authentication information can be embedded directly into the Stack Component, although this is not recommended for security reasons:
    ...
    ```
2.  [a ZenML secret](../../how-to/interact-with-secrets.md) can hold the AWS credentials and then be referenced in the S3 Artifact Store configuration attributes:
...
The AWS Service Connector discovered and lifted the AWS Secret Key that was configured on the local machine and securely stored it in the [Secrets Store](../../how-to/interact-with-secrets.md).
