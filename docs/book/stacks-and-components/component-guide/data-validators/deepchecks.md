...
Then, place the following snippet above your pipeline definition. Note that the path of the `dockerfile` are relative to where the pipeline definition file is. Read [the containerization guide](../../../how-to/containerize-your-pipeline/) for more details:
...
Alternatively, if you are running inside a Jupyter notebook, you can load and render the suites and results using the [artifact.visualize() method](../../../how-to/handle-data-artifacts/visualize-artifacts.md), e.g.:
