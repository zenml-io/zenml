...
{% hint style="info" %}
The output can be customized with an `--output` (json, yaml, csv, tsv, table) option and a `--columns` selection. For automation and coding agents, you can also enable `ZENML_CLI_MACHINE_MODE=true` to default commands that support `--output` to JSON and fail fast on interactive prompts; confirmation prompts also fail fast unless the command's explicit flag (usually `--yes`) is passed. See [environment variables](environment-variables.md#cli-output-formatting) for more details.
{% endhint %}
