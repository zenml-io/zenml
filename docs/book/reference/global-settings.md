...
3. The `zenml.db` in the `default_zen_store` directory is the default SQLite database where ZenML stores all information about the stacks, stack components, custom stack component flavors, etc.
    ...
    The database runs in [write-ahead logging](https://www.sqlite.org/wal.html) mode, so SQLite keeps two more files next to it while the database is in use: `zenml.db-wal` and `zenml.db-shm`. Recent commits can still be in `zenml.db-wal`, so copy all three files together when you back up or move the database. A copy of `zenml.db` alone can be missing the most recent changes.
