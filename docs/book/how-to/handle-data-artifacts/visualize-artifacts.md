...
Visualizations are usually stored alongside the artifact, in the [artifact store](../component-guide/artifact-stores/). Therefore, if a user would like to see the visualization displayed on the ZenML dashboard, they must give access to the server to connect to the artifact store.
