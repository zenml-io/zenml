...
When this pipeline is executed, the run of the pipeline gets logged to the ZenML dashboard where you can now go to look at its dag and all the associated metadata. To acces the dashboard you need to have a ZenML server either running locally or remotely. See our documentation on this [here](../../getting-started/why-deploy-zenml/).
