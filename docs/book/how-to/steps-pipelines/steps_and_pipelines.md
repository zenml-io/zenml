...
description: >-
  ...
  Learn how to create, configure, and run ML workflows with ZenML.
---
...
Steps are automatically cached based on their inputs. You can control caching behavior:
...
This creates an unlisted pipeline run with just that step. If you want to bypass ZenML and run the underlying function directly:
...
By default, ZenML determines step execution order based on data dependencies. You can explicitly control execution order with the `after` parameter:
...
def my_pipeline():
    ...
    # step_c will only run after both step_a and step_b complete
    step_c(after=[step_a_output, step_b_output])
```

You can also specify the step invocation ID directly:
...
You can compose pipelines from other pipelines:
...
You can configure automatic retries for steps that may encounter transient failures:
...
You can define hooks to execute before or after a pipeline or step:
...
@step(success_hooks=[success_hook], on_failure=failure_hook)
def my_step():
    return 42
```
...
You can access information about the current run using the step context:
...
ZenML provides built-in alerter hooks:
...
You can implement parallel processing patterns:
...
By default, ZenML uses the function name as the step invocation ID. You can specify a custom ID:
...
You can give your pipeline runs descriptive names:
