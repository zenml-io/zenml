    ...
    if version_folders[0] != latest_version:
        ...
        for group in new_version_groups:
            ...
            for page in group.get('pages', []):
                ...
                # check if the file path is in the all_mdx_files after removing the version from the file path
                file_path_without_version = file_path.split('/')[-1]
                if file_path_without_version in all_mdx_files:
                    page_path = page_path.split('/')[-1]
                    all_mdx_files.remove(file_path_without_version)  # Remove from the list as it's processed
                    full_file_path = f"docs/mintlify/{new_version}/{file_path_without_version}"
        ...
        mint_data['navigation'] = new_version_groups + [
            ...
            if group.get('version') != new_version
        ]
