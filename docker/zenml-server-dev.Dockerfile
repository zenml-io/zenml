...
RUN pip install --upgrade pip uv setuptools \
    && uv pip install .[server,secrets-aws,secrets-gcp,secrets-azure,secrets-hashicorp,s3fs,gcsfs,adlfs,connectors-aws,connectors-gcp,connectors-azure,azureml,sagemaker,vertex,native-schedules] "alembic==1.15.2" \
