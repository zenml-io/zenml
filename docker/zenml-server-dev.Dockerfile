...
RUN pip install --upgrade pip uv setuptools \
    && uv pip install .[server,server-streaming,otel,aws-rds-iam,secrets-aws,secrets-gcp,secrets-azure,secrets-hashicorp,s3fs,gcsfs,adlfs,connectors-aws,connectors-gcp,connectors-azure,azureml,sagemaker,vertex] "alembic==1.15.2" \
...
# in editable mode
RUN pip install --no-deps --no-cache -e .[server,server-streaming,otel,aws-rds-iam,secrets-aws,secrets-gcp,secrets-azure,secrets-hashicorp,s3fs,gcsfs,adlfs,connectors-aws,connectors-gcp,connectors-azure,azureml,sagemaker,vertex]
...
RUN pip install --no-deps --no-cache .[server,server-streaming,otel,aws-rds-iam,secrets-aws,secrets-gcp,secrets-azure,secrets-hashicorp,s3fs,gcsfs,adlfs,connectors-aws,connectors-gcp,connectors-azure,azureml,sagemaker,vertex] \
